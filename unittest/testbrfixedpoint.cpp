...
static uint_t BURGER_API TestBitReverse_32(void)
{
    uint_t uFailure = FALSE;
    uint_t uBitCount = 1;
    do {
        ...
        do {
            uint32_t uReturn = Burger::BitReverse(pWork->m_uInput, uBitCount);
            uint32_t uToTest = pWork->m_uOutput >> (32 - uBitCount);
            uint_t uTest = uReturn != uToTest;
            uFailure |= uTest;
            ReportFailure(
...
static uint_t BURGER_API TestBitReverse_64(void)
{
    uint_t uFailure = FALSE;
    uint_t uBitCount = 1;
    do {
        ...
        do {
            uint64_t uReturn = Burger::BitReverse(pWork->m_uInput, uBitCount);
            uint64_t uToTest = pWork->m_uOutput >> (64 - uBitCount);
            uint_t uTest = uReturn != uToTest;
            uFailure |= uTest;
            if (uTest) {
...
static uint_t BURGER_API TestBitSetCount_32(void)
{
    uint_t uFailure = FALSE;
    const WordTest32_t* pWork = BitSetCount_32Table;
    uintptr_t uCount = BURGER_ARRAYSIZE(BitSetCount_32Table);
    do {
        uint_t uReturn = Burger::BitSetCount(pWork->m_uInput);
        uint_t uToTest = static_cast<uint_t>(pWork->m_uOutput);
        uint_t uTest = uReturn != uToTest;
        uFailure |= uTest;
...
static uint_t BURGER_API TestBitSetCount_64(void)
{
    uint_t uFailure = FALSE;
...
static uint_t BURGER_API TestGetLoWord(void)
{
    uint_t uFailure = FALSE;
    const WordTest32_t* pWork = GetLoWordTable;
    uintptr_t uCount = BURGER_ARRAYSIZE(GetLoWordTable);
    do {
        uint32_t uReturn = Burger::GetLoWord(pWork->m_uInput);
        uint_t uTest = uReturn != pWork->m_uOutput;
        uFailure |= uTest;
...
static uint_t BURGER_API TestGetHiWord(void)
{
    uint_t uFailure = FALSE;
    const WordTest32_t* pWork = GetHiWordTable;
    uintptr_t uCount = BURGER_ARRAYSIZE(GetHiWordTable);
    do {
        uint32_t uReturn = Burger::GetHiWord(pWork->m_uInput);
        uint_t uTest = uReturn != pWork->m_uOutput;
        uFailure |= uTest;
...
static uint_t BURGER_API TestFloatToIntRoundToZero(void)
{
    uint_t uFailure = FALSE;
    const OldFloatToIntTest32_t* pWork = FloatToIntTestTable;
    uintptr_t uCount = BURGER_ARRAYSIZE(FloatToIntTestTable);
    do {
        int32_t iReturn = Burger::FloatToIntRoundToZero(pWork->m_fInput);
        uint_t uTest = iReturn != pWork->m_iOutput;
        uFailure |= uTest;
        ReportFailure(
...
static uint_t BURGER_API TestFloatToFixedNearest(void)
{
    uint_t uFailure = FALSE;
    ...
    do {
        int32_t iReturn = Burger::FloatToFixedNearest(pWork->m_fInput);
        uint_t uTest = iReturn != pWork->m_iOutput;
        uFailure |= uTest;
        ReportFailure(
...
static uint_t BURGER_API TestAbs64(void)
{
    uint_t uFailure = FALSE;
    const IntTest64_t* pWork = AbsTestTable64;
    uintptr_t uCount = BURGER_ARRAYSIZE(AbsTestTable64);
    do {
        int64_t iReturn = Burger::Abs(pWork->m_iInput);
        uint_t uTest = iReturn != pWork->m_iOutput;
        uFailure |= uTest;
        if (uTest) {
            Burger::NumberStringHex Text1(pWork->m_iInput);
            Burger::NumberStringHex Text2(iReturn);
            Burger::NumberStringHex Text3(pWork->m_iOutput);
...
static const IntTest32_t SignTestTable32[] = {{0x00000000, 0}, {0x00000001, 1},
    ...
    {static_cast<int32_t>(0x80000001), -1}, {static_cast<int32_t>(0xC0000000), -1},
    {static_cast<int32_t>(0xFFFFFFFF), -1}};

static uint_t BURGER_API TestSign32(void)
{
    uint_t uFailure = FALSE;
    const IntTest32_t* pWork = SignTestTable32;
    uintptr_t uCount = BURGER_ARRAYSIZE(SignTestTable32);
    do {
        int32_t iReturn = Burger::Sign(pWork->m_iInput);
        uint_t uTest = iReturn != pWork->m_iOutput;
        uFailure |= uTest;
...
static uint_t BURGER_API TestSqrt32(void)
{
    uint_t uFailure = FALSE;
    const WordTest32_t* pWork = Sqrt32TestTable;
    uintptr_t uCount = BURGER_ARRAYSIZE(Sqrt32TestTable);
    do {
        uint32_t uReturn = Burger::Sqrt(pWork->m_uInput);
        uint_t uTest = uReturn != pWork->m_uOutput;
        uFailure |= uTest;
        ReportFailure("Burger::Sqrt((uint32_t)0x%08X) = 0x%08X, expected 0x%08X",
            uTest, pWork->m_uInput, uReturn, pWork->m_uOutput);
        ++pWork;
    } while (--uCount);
    return uFailure;
...
static uint_t BURGER_API TestSqrtFixedToWord32(void)
{
    uint_t uFailure = FALSE;
    const FixedTest32_t* pWork = SqrtFixedToWord32TestTable;
    uintptr_t uCount = BURGER_ARRAYSIZE(SqrtFixedToWord32TestTable);
    do {
        uint32_t uReturn = Burger::SqrtFixedToWord32(pWork->m_iInput);
        uint_t uTest = uReturn != static_cast<uint32_t>(pWork->m_iOutput);
        uFailure |= uTest;
        ReportFailure(
...
static uint_t BURGER_API TestSqrtFixed32(void)
{
    uint_t uFailure = FALSE;
...
int BURGER_API TestBrfixedpoint(uint_t uVerbose)
{
