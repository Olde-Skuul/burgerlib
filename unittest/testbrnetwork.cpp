...
static void BURGER_API DisplayStats(
	const Burger::NetworkManager* pNet) BURGER_NOEXCEPT
{
	...
	if (uAddressCount) {
		...
		do {
			...
			Message("Local Address = %s", TempStringNetAddr.c_str());
			++pNetAddr;
		} while (--uAddressCount);
	}
...
static uint_t BURGER_API TestIPv4ToString(void) BURGER_NOEXCEPT
{
	...
	do {
		...
		ReportFailure(
			...
			uTest, pWork->m_uIP, pWork->m_uPort, StringTest.c_str(),
			pWork->m_pString);
		if (!uTest) {
			uint32_t IPv4;
			uint_t Port;
			uTest = static_cast<uint_t>(
				Burger::StringToIPv4(StringTest.c_str(), &IPv4, &Port));
			if (!uTest) {
			...
			ReportFailure(
				...
				uTest, StringTest.c_str(), IPv4, Port, pWork->m_uIP,
				pWork->m_uPort);
		}
		++pWork;
	} while (--uCount);
...
static uint_t BURGER_API TestIPv6ToString(void) BURGER_NOEXCEPT
{
	Burger::String StringTest;
	uint_t uFailure = FALSE;
...
static void BURGER_API TestIPv4Resolve(
	Burger::NetworkManager* pNet) BURGER_NOEXCEPT
{
...
static void BURGER_API TestIPv6Resolve(
	Burger::NetworkManager* pNet) BURGER_NOEXCEPT
{
	Burger::NetAddr_t IPAddress;
	Burger::String StringTest;
...
static void BURGER_API TestGetIPv4Address(
	Burger::NetworkManager* pNet) BURGER_NOEXCEPT
{
	Burger::NetAddr_t IPAddress;
	Burger::String StringTest;
...
static void BURGER_API TestGetIPv6Address(
	Burger::NetworkManager* pNet) BURGER_NOEXCEPT
{
	Burger::NetAddr_t IPAddress;
	Burger::String StringTest;
...
int BURGER_API TestNetwork(uint_t uVerbose) BURGER_NOEXCEPT
{
