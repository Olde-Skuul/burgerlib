...
Debuglnx:
	...
	@$(MAKE) -e CONFIG=Debug TARGET=lnx -f burgermaklnx.mak $(DESTINATION_DIR)/lib$(PROJECT_NAME)maklnxdbg.a

.PHONY: Internallnx
Internallnx:
	@-mkdir -p "$(DESTINATION_DIR)"
	@-mkdir -p "$(BASE_TEMP_DIR)maklnxint"
	@$(MAKE) -e CONFIG=Internal TARGET=lnx -f burgermaklnx.mak $(DESTINATION_DIR)/lib$(PROJECT_NAME)maklnxint.a

.PHONY: Releaselnx
Releaselnx:
	@-mkdir -p "$(DESTINATION_DIR)"
	@-mkdir -p "$(BASE_TEMP_DIR)maklnxrel"
	@$(MAKE) -e CONFIG=Release TARGET=lnx -f burgermaklnx.mak $(DESTINATION_DIR)/lib$(PROJECT_NAME)maklnxrel.a
...
$(DESTINATION_DIR)/lib$(PROJECT_NAME)maklnxdbg.a: $(OBJS) burgermaklnx.mak
	@ar -rcs $@ $(TRUE_OBJS)
	@-p4 edit "$(BURGER_SDKS)\linux\burgerlib/$(@F)"
	@-cp -T "$@" "$(BURGER_SDKS)\linux\burgerlib/$(@F)"
	@-p4 revert -a "$(BURGER_SDKS)\linux\burgerlib/$(@F)"

$(DESTINATION_DIR)/lib$(PROJECT_NAME)maklnxint.a: $(OBJS) burgermaklnx.mak
	@ar -rcs $@ $(TRUE_OBJS)
	@-p4 edit "$(BURGER_SDKS)\linux\burgerlib/$(@F)"
	@-cp -T "$@" "$(BURGER_SDKS)\linux\burgerlib/$(@F)"
	@-p4 revert -a "$(BURGER_SDKS)\linux\burgerlib/$(@F)"

$(DESTINATION_DIR)/lib$(PROJECT_NAME)maklnxrel.a: $(OBJS) burgermaklnx.mak
	@ar -rcs $@ $(TRUE_OBJS)
	@-p4 edit "$(BURGER_SDKS)\linux\burgerlib/$(@F)"
	@-cp -T "$@" "$(BURGER_SDKS)\linux\burgerlib/$(@F)"
	@-p4 revert -a "$(BURGER_SDKS)\linux\burgerlib/$(@F)"
