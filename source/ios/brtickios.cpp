...
uint32_t BURGER_API Burger::Tick::Read(void) BURGER_NOEXCEPT
{
	...
	if (!bStarted60Hz) {
		...
		uStep60Hz = (50000000ULL * static_cast<uint64_t>(sTimebaseInfo.denom)) /
			static_cast<uint64_t>(sTimebaseInfo.numer * 3);
		return 1;
	}
...
uint32_t BURGER_API Burger::Tick::ReadMicroseconds(void) BURGER_NOEXCEPT
{
	...
	if (!bStarted1MHz) {
		...
		mach_timebase_info_data_t sTimebaseInfo;
		mach_timebase_info(&sTimebaseInfo);
		uStep1MHz = (1000ULL * static_cast<uint64_t>(sTimebaseInfo.denom)) /
			static_cast<uint64_t>(sTimebaseInfo.numer);
		return 1;
	}
...
uint32_t BURGER_API Burger::Tick::ReadMilliseconds(void) BURGER_NOEXCEPT
{
	...
	if (!bStarted1KHz) {
		...
		mach_timebase_info_data_t sTimebaseInfo;
		mach_timebase_info(&sTimebaseInfo);
		uStep1KHz = (1000000ULL * static_cast<uint64_t>(sTimebaseInfo.denom)) /
			static_cast<uint64_t>(sTimebaseInfo.numer);
		return 1;
	}
...
Burger::FloatTimer::FloatTimer() BURGER_NOEXCEPT: m_bPaused(FALSE)
{
	...
	m_dReciprocalFrequency = (static_cast<double>(sTimebaseInfo.numer) /
								 static_cast<double>(sTimebaseInfo.denom)) /
		1000000000.0;
