...
static const int g_Protocols[8] = {
	AF_INET,IPPROTO_UDP,
...
/*! ************************************
	...
	\return Zero if no error, non-zero if not supported or unimplemented protocol
	\sa FromSocketAddr(const sockaddr *), ToSocketAddr(sockaddr_at *) const, ToSocketAddr(sockaddr_ipx *) const,
		ToSocketAddr(sockaddr_in *) const or ToSocketAddr(sockaddr_in6 *) const
...
Burger::eError BURGER_API Burger::NetAddr_t::ToSocketAddr(sockaddr *pOutput) const
{
	...
	case TYPE_IPV4:
		...
		reinterpret_cast<sockaddr_in *>(pOutput)->sin_port = BigEndian::Load(static_cast<Word16>(U.IPv4.m_uPort));
		reinterpret_cast<sockaddr_in *>(pOutput)->sin_addr.s_addr = BigEndian::Load(U.IPv4.m_uIP);
		MemoryClear(reinterpret_cast<sockaddr_in *>(pOutput)->sin_zero,sizeof(reinterpret_cast<sockaddr_in *>(pOutput)->sin_zero));
		break;

	case TYPE_IPV6:
		pOutput->sa_family = AF_INET6;
		reinterpret_cast<sockaddr_in6 *>(pOutput)->sin6_port = BigEndian::Load(static_cast<Word16>(U.IPv6.m_uPort));
		reinterpret_cast<sockaddr_in6 *>(pOutput)->sin6_flowinfo = 0;
		MemoryCopy(reinterpret_cast<sockaddr_in6 *>(pOutput)->sin6_addr.u.Byte,U.IPv6.m_IP,sizeof(reinterpret_cast<sockaddr_in6 *>(pOutput)->sin6_addr.u.Byte));
		reinterpret_cast<sockaddr_in6 *>(pOutput)->sin6_scope_id = 0;
		break;

	case TYPE_IPX:
		pOutput->sa_family = AF_IPX;
		reinterpret_cast<sockaddr_ipx *>(pOutput)->sa_socket = BigEndian::Load(static_cast<Word16>(U.IPX.m_uSocket));
		MemoryCopy(reinterpret_cast<sockaddr_ipx *>(pOutput)->sa_netnum,U.IPX.m_Net,4);
		MemoryCopy(reinterpret_cast<sockaddr_ipx *>(pOutput)->sa_nodenum,U.IPX.m_Node,6);
		break;

	case TYPE_APPLETALK:
		pOutput->sa_family = AF_APPLETALK;
		reinterpret_cast<sockaddr_at *>(pOutput)->sat_net = BigEndian::Load(static_cast<Word16>(U.APPLETALK.m_uNetwork));
		reinterpret_cast<sockaddr_at *>(pOutput)->sat_node = static_cast<Word8>(U.APPLETALK.m_uNodeID);
		reinterpret_cast<sockaddr_at *>(pOutput)->sat_socket = static_cast<Word8>(U.APPLETALK.m_uSocket);
		break;
...
/*! ************************************
	...
	\fn Burger::eError Burger::NetAddr_t::ToSocketAddr(sockaddr_at *pOutput) const
...
/*! ************************************
	...
	\fn Burger::eError Burger::NetAddr_t::ToSocketAddr(sockaddr_in *pOutput) const
	\brief Calls ToSocketAddr(sockaddr *) const
...
/*! ************************************
	...
	\fn Burger::eError Burger::NetAddr_t::ToSocketAddr(sockaddr_in6 *pOutput) const
	\brief Calls ToSocketAddr(sockaddr *) const
...
/*! ************************************
	...
	\fn Burger::eError Burger::NetAddr_t::ToSocketAddr(sockaddr_ipx *pOutput) const
	\brief Calls ToSocketAddr(sockaddr *) const
...
/*! ************************************
	...
	\fn Burger::eError Burger::NetAddr_t::ToSocketAddr(sockaddr_storage *pOutput) const
	\brief Calls ToSocketAddr(sockaddr *) const
...
/*! ************************************
	...
	\return Zero if no error, non-zero if not supported or unimplemented protocol
	\sa ToSocketAddr(sockaddr *) const, FromSocketAddr(const sockaddr_at *), FromSocketAddr(const sockaddr_ipx *),
	FromSocketAddr(const sockaddr_in *) or FromSocketAddr(const sockaddr_in6 *)
...
Burger::eError BURGER_API Burger::NetAddr_t::FromSocketAddr(const sockaddr *pInput)
{
	...
	case AF_INET:
		m_uType=TYPE_IPV4;
		U.IPv4.m_uPort=BigEndian::Load(reinterpret_cast<const sockaddr_in *>(pInput)->sin_port);
		U.IPv4.m_uIP=BigEndian::Load(static_cast<Word32>(reinterpret_cast<const sockaddr_in *>(pInput)->sin_addr.s_addr));
		break;

	case AF_INET6:
		m_uType=TYPE_IPV6;
		U.IPv6.m_uPort=BigEndian::Load(reinterpret_cast<const sockaddr_in6 *>(pInput)->sin6_port);
		MemoryCopy(U.IPv6.m_IP,reinterpret_cast<const sockaddr_in6 *>(pInput)->sin6_addr.u.Byte,sizeof(U.IPv6.m_IP));
		break;

	case AF_IPX:
		m_uType=TYPE_IPX;
		U.IPX.m_uSocket=BigEndian::Load(reinterpret_cast<const sockaddr_ipx *>(pInput)->sa_socket);
		MemoryCopy(U.IPX.m_Net,reinterpret_cast<const sockaddr_ipx *>(pInput)->sa_netnum,4);
		MemoryCopy(U.IPX.m_Node,reinterpret_cast<const sockaddr_ipx *>(pInput)->sa_nodenum,6);
		break;

	case AF_APPLETALK:
		m_uType=TYPE_APPLETALK;
		U.APPLETALK.m_uNetwork = BigEndian::Load(reinterpret_cast<const sockaddr_at *>(pInput)->sat_net);
		U.APPLETALK.m_uNodeID = reinterpret_cast<const sockaddr_at *>(pInput)->sat_node;
		U.APPLETALK.m_uSocket = reinterpret_cast<const sockaddr_at *>(pInput)->sat_socket;
		break;
...
/*! ************************************
	...
	\fn Burger::eError Burger::NetAddr_t::FromSocketAddr(const sockaddr_in *pInput)
...
/*! ************************************
	...
	\fn Burger::eError Burger::NetAddr_t::FromSocketAddr(const sockaddr_in6 *pInput)
	\brief Calls FromSocketAddr(const sockaddr *)
...
/*! ************************************
	...
	\fn Burger::eError Burger::NetAddr_t::FromSocketAddr(const sockaddr_at *pInput)
	\brief Calls FromSocketAddr(const sockaddr *)
...
/*! ************************************
	...
	\fn Burger::eError Burger::NetAddr_t::FromSocketAddr(const sockaddr_ipx *pInput)
	\brief Calls FromSocketAddr(const sockaddr *)
...
/*! ************************************
	...
	\fn Burger::eError Burger::NetAddr_t::FromSocketAddr(const sockaddr_storage *pInput)
	\brief Calls FromSocketAddr(const sockaddr *)
...
Burger::eError BURGER_API Burger::NetworkManager::Init(void)
{
	...
	if (!m_bWSAStarted) {
		...
		if (uResult== kErrorNone) {
			...
			Word i = 0;
			do {
				SocketType uSocket = socket(g_Protocols[i],SOCK_DGRAM,g_Protocols[i+1]);
...
/***************************************
	...
	\note If the DNS address is a port literal, it will be decoded without contacting
	any server.

	\param pOutput Pointer to the NetAddr_t structure that will contain the resolved address
	\param pDNSName Pointer to a "C" string in UTF-8 encoding to pass to the DNS server
		...
		if (TempDNS.IsValid()) {
			...
			if (uResult== kErrorNone) {
				...
				if (uResult!= kErrorNone) {
					...
					if (uResult== kErrorNone) {
						uIPv4 = BigEndian::Load(static_cast<Word32>(reinterpret_cast<sockaddr_in*>(pResult->ai_addr)->sin_addr.s_addr));
						...
						if (pWork) {
							do {
								if (pWork->ai_family == AF_INET6) {
									const sockaddr_in6* pIP6Sock = reinterpret_cast<const sockaddr_in6*>(pWork->ai_addr);
									if (pIP6Sock) {
										// Copy the address
										MemoryCopy(pOutput->U.IPv6.m_IP,&pIP6Sock->sin6_addr,16);
										// No error
										uResult = kErrorNone;
										break;
									}
...
/***************************************
	...
	\param pDestination Pointer to the NetAddr_t that has the destination address
...
Burger::eError BURGER_API Burger::NetworkManager::SendPacket(const NetAddr_t *pDestination,const void *pBuffer,WordPtr uBufferSize)
{
	eError uResult = kErrorNone;
	...
	if (uBufferSize) {
		...
		switch (uType) {
		default:
			uResult = kErrorNotSupportedOnThisPlatform;		// This isn't a type supported on Windows
			break;
		...
		case NetAddr_t::TYPE_IPV6:
			...
			if (uBufferSize>(65535U-28U)) {
				uResult = kErrorBufferTooLarge;	// Packet is too big for this protocol
			}
		...
		if (uResult== kErrorNone) {
			...
			if (uSocket==INVALID_SOCKET) {
				...
				if (pDestination->IsBroadcast()) {
					...
					int iOption = TRUE;
					if (setsockopt(uSocket,SOL_SOCKET,SO_BROADCAST,reinterpret_cast<char*>(&iOption),sizeof(iOption))==SOCKET_ERROR) {
						// Woah, the call FAILED?!?!? How?
						uResult = kErrorSocketFailure;
					}
				}

				// If broadcast upgrade wasn't needed or succeeded, proceed to transmit the data
				if (uResult== kErrorNone) {
					...
					int iSendtoError = sendto(uSocket,static_cast<const char *>(pBuffer),static_cast<int>(uBufferSize),0,&Address.m_Base,iAddressSize);
				...
				if (closesocket(uSocket)==SOCKET_ERROR) {
					...
					if (uResult== kErrorNone) {
						// Mark the error (Couldn't post it in the transmission queue?)
						uResult = kErrorSocketFailure;
					}
				}
			}
...
/***************************************
	...
	\brief Send a data packet to another network address using guaranteed protocol
...
Burger::eError BURGER_API Burger::NetworkManager::SendStream(const NetAddr_t *pDestination,const void *pBuffer,WordPtr uBufferSize)
{
	eError uResult = kErrorNone;
	...
	if (uBufferSize) {
		...
		if (uResult== kErrorNone) {
			...
			if (uSocket==INVALID_SOCKET) {
				...
				if (connect(uSocket,&Address.m_Base,iAddressSize)==SOCKET_ERROR) {
				...
				if (uResult== kErrorNone) {
					...
					int iSendError = send(uSocket,static_cast<const char *>(pBuffer),static_cast<int>(uBufferSize),0);
...
Burger::eError BURGER_API Burger::NetworkManager::EnumerateLocalAddresses(void)
{
	...
	const ULONG uFlags = GAA_FLAG_INCLUDE_PREFIX|GAA_FLAG_SKIP_MULTICAST|GAA_FLAG_SKIP_DNS_SERVER|GAA_FLAG_SKIP_FRIENDLY_NAME;
	...
	if (uResult==ERROR_BUFFER_OVERFLOW) {
		...
		if (uSize) {
			...
			IP_ADAPTER_ADDRESSES *pAdapterAddresses = static_cast<IP_ADAPTER_ADDRESSES *>(Alloc(uSize));
			if (pAdapterAddresses) {
					
				// Grab the list
				uResult = GetAdaptersAddresses(AF_UNSPEC,uFlags,NULL,pAdapterAddresses,&uSize);
				if (uResult==ERROR_SUCCESS) {
				
					// Iterate over the list to get the number of entries that will be generated
					...
					do {
						...
						if ((pWork->IfType==IF_TYPE_ETHERNET_CSMACD) ||
							...
							const IP_ADAPTER_UNICAST_ADDRESS *pUnicast = pWork->FirstUnicastAddress;
							if (pUnicast) {
								do {
					...
					if (uCount) {
						...
						NetAddr_t *pNetAddr = static_cast<NetAddr_t *>(Alloc(sizeof(NetAddr_t)*uCount));
                        uError = kErrorOutOfMemory;
						if (pNetAddr) {
							...
							do {
								...
								if ((pWork->IfType==IF_TYPE_ETHERNET_CSMACD) ||
									...
									if (pUnicast) {
										do {
											if (pUnicast->Flags & IP_ADAPTER_ADDRESS_DNS_ELIGIBLE) {
												// Convert the sockaddr into a NetAddr_t
												pNetAddr->FromSocketAddr(pUnicast->Address.lpSockaddr);
												++pNetAddr;
											}
											pUnicast = pUnicast->Next;
										} while (pUnicast);
...
Burger::eError BURGER_API Burger::SocketTestForReading(SocketType uSocket)
{
	...
	int iAvailable = recvfrom(uSocket,Buffer,sizeof(Buffer),MSG_PEEK,NULL,NULL);
	...
	if (iAvailable == -1) {
		...
		if ((iError != WSAEMSGSIZE) &&
			...
			iAvailable = recvfrom(uSocket,Buffer,sizeof(Buffer),0,NULL,NULL);
		}
	} 
