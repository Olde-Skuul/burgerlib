...
/*! ************************************
	...
	in the form of a single extra pointer in the VTable and a single
	instance of piece of code returning a pointer.

	The class itself does not contain any additional data to support
	this feature!!!

	By using macros for calls, data tables and class hooks, a simple
	way to check if a base class is really a specific class is
	created.
	...
	Debug::Message("foo pBar %u\n",BURGER_STATICRTTI_ISTYPE(foo,pBar));			// TRUE
	Debug::Message("foo2 pBar %u\n",BURGER_STATICRTTI_ISTYPE(foo2,pBar));		// FALSE
	Debug::Message("foo3 pBar %u\n",BURGER_STATICRTTI_ISTYPE(foo3,pBar));		// FALSE
	Debug::Message("foo4 pBar %u\n",BURGER_STATICRTTI_ISTYPE(foo4,pBar));		// FALSE
	Debug::Message("foo5 pBar %u\n\n",BURGER_STATICRTTI_ISTYPE(foo5,pBar));		// FALSE

	Debug::Message("foo pBar2 %u\n",BURGER_STATICRTTI_ISTYPE(foo,pBar2));		// TRUE
	Debug::Message("foo2 pBar2 %u\n",BURGER_STATICRTTI_ISTYPE(foo2,pBar2));		// TRUE
	Debug::Message("foo3 pBar2 %u\n",BURGER_STATICRTTI_ISTYPE(foo3,pBar2));		// FALSE
	Debug::Message("foo4 pBar2 %u\n",BURGER_STATICRTTI_ISTYPE(foo4,pBar2));		// FALSE
	Debug::Message("foo5 pBar2 %u\n\n",BURGER_STATICRTTI_ISTYPE(foo5,pBar2));	// FALSE

	Debug::Message("foo pBar3 %u\n",BURGER_STATICRTTI_ISTYPE(foo,pBar3));		// TRUE
	Debug::Message("foo2 pBar3 %u\n",BURGER_STATICRTTI_ISTYPE(foo2,pBar3));		// TRUE
	Debug::Message("foo3 pBar3 %u\n",BURGER_STATICRTTI_ISTYPE(foo3,pBar3));		// TRUE
	Debug::Message("foo4 pBar3 %u\n",BURGER_STATICRTTI_ISTYPE(foo4,pBar3));		// FALSE
	Debug::Message("foo5 pBar3 %u\n\n",BURGER_STATICRTTI_ISTYPE(foo5,pBar3));	// FALSE

	Debug::Message("foo pBar4 %u\n",BURGER_STATICRTTI_ISTYPE(foo,pBar4));		// TRUE
	Debug::Message("foo2 pBar4 %u\n",BURGER_STATICRTTI_ISTYPE(foo2,pBar4));		// TRUE
	Debug::Message("foo3 pBar4 %u\n",BURGER_STATICRTTI_ISTYPE(foo3,pBar4));		// FALSE
	Debug::Message("foo4 pBar4 %u\n",BURGER_STATICRTTI_ISTYPE(foo4,pBar4));		// TRUE
	Debug::Message("foo5 pBar4 %u\n\n",BURGER_STATICRTTI_ISTYPE(foo5,pBar4));	// FALSE

	Debug::Message("foo pBar5 %u\n",BURGER_STATICRTTI_ISTYPE(foo,pBar5));		// TRUE
	Debug::Message("foo2 pBar5 %u\n",BURGER_STATICRTTI_ISTYPE(foo2,pBar5));		// FALSE
	Debug::Message("foo3 pBar5 %u\n",BURGER_STATICRTTI_ISTYPE(foo3,pBar5));		// FALSE
	Debug::Message("foo4 pBar5 %u\n",BURGER_STATICRTTI_ISTYPE(foo4,pBar5));		// FALSE
	Debug::Message("foo5 pBar5 %u\n\n",BURGER_STATICRTTI_ISTYPE(foo5,pBar5));	// TRUE
	...
	\sa BURGER_RTTICAST, BURGER_RTTICONSTCAST, BURGER_STATICRTTI_ISTYPE, BURGER_RTTI_IN_CLASS,
...
/*! ************************************
	...
	Walk the StaticRTTI linked list from the most derived type
	and up to the base class, all the while checking for
	a match.
...
/*! ************************************
	...
	For a class that's not derived from anything, use this macro to generate
	a terminating linked list entry

	\param __ClassName Class name (No quotes)

	\sa Burger::StaticRTTI, BURGER_RTTI_IN_CLASS or BURGER_CREATE_STATICRTTI_PARENT
...
/*! ************************************
	...
	For a class that's derived, use this macro to generate
	a linked list entry that connects to its parent

	\param __ClassName Class name (No quotes)
	\param __ParentClass Parent class name (No quotes)
	\sa Burger::StaticRTTI, BURGER_RTTI_IN_CLASS or BURGER_CREATE_STATICRTTI_BASE
...
/*! ************************************
	...
	This macro will use the magic data in the class to find if its
	really the requested class.
...
/*! ************************************
	...
	Given a derived class, check if the pointer is part of the derived
	class and return a properly cast pointer. If the test fails, return
	\ref NULL.
...
/*! ************************************
	...
	Given a derived class, check if the pointer is part of the derived
	class and return a properly cast const pointer. If the test fails, return
	\ref NULL.
