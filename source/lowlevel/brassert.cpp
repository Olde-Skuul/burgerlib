...
/*! ************************************
	...
	\param conditional A boolean that evaluates to \ref FALSE to call the assert
		function \ref Burger::Assert(const char *,const char *,uint_t)
...
/*! ************************************
	...
	\note This macro will always invoke the conditional and will return the
		...
		a Debug build, the \ref Burger::Assert(const char *,const char *,uint_t)
...
/*! ************************************
	...
	an \ref Assert(const char *,const char *,uint_t) call.
...
/*! ************************************
	...
	The normal behavior for \ref Assert(const char *,const char *,uint_t) is to
	call printf() with the failure condition and then call \ref Debug::Fatal().
	This behavior can be overridden by passing	a new function pointer via this
	call.

	The default behavior can be restored by passing \ref NULL for the pAssert
	parameter.

	\param pCallback Function to call when an assert is triggered, \ref NULL
		restores defaults
...
/*! ************************************
	...
	calls \ref Halt(). If execution continues, it will call Debug::Fatal() with
	the same message and exit the application.
...
void BURGER_API Burger::Halt(void) BURGER_NOEXCEPT
{
