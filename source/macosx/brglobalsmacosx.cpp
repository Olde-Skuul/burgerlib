...
/*! ************************************
	...
	Given a CGDirectDisplayID, return the name of a monitor.
 	If the monitor has no name, in cases such as if it's a
 	Mac without a monitor, it will be set to "Inactive"
...
{
	CFDictionaryRef rInfoDictionary = IODisplayCreateInfoDictionary(CGDisplayIOServicePort(uDisplayID),
		kIODisplayOnlyPreferredName);

	CFDictionaryRef rNameList = static_cast<CFDictionaryRef>(CFDictionaryGetValue(rInfoDictionary,
		CFSTR(kDisplayProductName)));

	CFStringRef rMonitorName;
	if (!rNameList || !CFDictionaryGetValueIfPresent(rNameList,CFSTR("en_US"),
...
/*! ************************************
	...
	Given a CFDictionaryRef and a query key, look up the item in
	the dictionary and if present, return the value as an
	integer. Returns zero on failure.
...
int Burger::Globals::NumberFromKey(CFDictionaryRef pDictionary,const char *pKey)
{
	// Convert the key to a string
	CFStringRef rString = CFStringCreateWithCStringNoCopy(NULL,pKey,kCFStringEncodingUTF8,kCFAllocatorNull);

	// Perform the query
	CFNumberRef pValue = static_cast<CFNumberRef>(CFDictionaryGetValue(pDictionary,rString));
