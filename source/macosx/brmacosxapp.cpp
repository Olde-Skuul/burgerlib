...
/***************************************
	...
	Create an NSResponder which will track
    events that the window is interested in
...
context:(void *)context
{
    ...
    if (object == m_pParent->GetWindow() && [keyPath isEqualToString:@"visible"]) {
...
//    SDL_Window *window = _data->window;
    ...
    if (pendingWindowOperation == PENDING_GOFULLSCREEN) {
        ...
        if (m_pParent->GetDisplay()->GetFlags() & Burger::Display::ALLOWRESIZING) {
