...
/*! ************************************
	...
	\fn Burger::ConsoleApp::WasDesktopLaunched(void) const
	\brief Was the application launched from the desktop

	Returns \ref TRUE if the console application was launched
	by double clicking on the icon or \ref FALSE if the 
	application was launched from a command line shell.
	
	\return \ref TRUE if desktop launched, \ref FALSE if from a shell.
	\sa Burger::ConsoleApp::GetArgv() const or Burger::ConsoleApp::GetArgc() const
...
/*! ************************************
	...
	\return \ref TRUE if the function pointer was called. \ref FALSE if normal processing should occur.
...
/*! ************************************
	...
	If the console application was invoked with argc == 2
	or argc == 3, then execute the callback with a
	Burgerlib input filename and a NULL for the output
	filename (argc==2) or a burgerlib input filename
	and a burgerlib output filename (argc==3). There is 
	no checking performed on the filenames to see if they
	are actually pointing to valid files.

	If argc!=2 and argc!=3 then set the error code to 10 and then the 
	pUsage function is called so that instructions can be displayed. If pUsage is NULL,
	no instructions will be printed.
	...
	\sa int Burger::ConsoleApp::SimpleTool(Burger::ConsoleApp::CallbackProc,Burger::ConsoleApp::UsageProc)

***************************************/

int BURGER_API Burger::ConsoleApp::InputAndOutput(Burger::ConsoleApp::CallbackProc pCallback,Burger::ConsoleApp::UsageProc pUsage)
{
	int iArgc = GetArgc();		// Get the number of arguments remaining
	int iResult;
...
/*! ************************************
	...
	For console applications that only accept 2 or 3 parameters,
	namely a file to input data and a file to output data,
	this helper function performs the drag and drop and
	mundane operations for setup and teardown of the command line.
	
	All is needed are two callbacks, one for the actual console
	application's conversion function and the other for 
	printing the instructions to the console.
	
	\param pCallback Pointer to a function to perform the actual work.
	\param pUsage Pointer to a function to print the instructions.
	\return The current error code. Can be returned to the operating system.
	\sa int Burger::ConsoleApp::InputAndOutput(Burger::ConsoleApp::CallbackProc pCallback,Burger::ConsoleApp::UsageProc pUsage)

***************************************/

int BURGER_API Burger::ConsoleApp::SimpleTool(Burger::ConsoleApp::CallbackProc pCallback,Burger::ConsoleApp::UsageProc pUsage)
{
	// Handle drag and drop
	if (!ProcessFilenames(pCallback)) {
		InputAndOutput(pCallback,pUsage);		// Handle the command line
	}
	PauseOnError();			// If an error occurred, pause the display if drag and drop
