...
private:
	struct Chunk_t {
		...
		uint8_t m_Buffer[kChunkSize]; ///< Chunk data
	};
...
public:
	...
	OutputMemoryStream(const OutputMemoryStream& rInput) BURGER_NOEXCEPT;
	OutputMemoryStream& operator=(
		const OutputMemoryStream& rInput) BURGER_NOEXCEPT;

#if defined(BURGER_RVALUE_REFERENCES)
	OutputMemoryStream(OutputMemoryStream&& rInput) BURGER_NOEXCEPT;
	OutputMemoryStream& operator=(OutputMemoryStream&& rInput) BURGER_NOEXCEPT;
#endif
	...
	{
		return AppendCString(pString->c_str());
	}
