...
Word32 Burger::GetID3V2TagLength(const void *pInput)
{
	Word uResult = (static_cast<const Word8 *>(pInput)[0] & 0x7FU)<<21U;
	uResult += (static_cast<const Word8 *>(pInput)[1] & 0x7FU)<<14U;
	uResult += (static_cast<const Word8 *>(pInput)[2] & 0x7FU)<<7U;
	uResult += (static_cast<const Word8 *>(pInput)[3] & 0x7FU);
	return uResult;
}
