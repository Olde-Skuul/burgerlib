...
Burger::OutputMemoryStream::OutputMemoryStream() BURGER_NOEXCEPT
    : m_pRoot(nullptr),
      m_pCurrent(nullptr),
      m_uIndex(CHUNKSIZE), // Will force Append(uint8_t) to add a new buffer
      m_uFileSize(0),
      m_uError(kErrorNone)
{
}
    ...
    if (!uResult) {
        ...
        if (!uResult) {
            ...
            if (uRemaining) {
                const Chunk_t* pChunk = m_pRoot;
                do {
                    uintptr_t uChunkSize = CHUNKSIZE;
        ...
        if (uRemaining) {
            ...
            do {
                ...
                if (uEndMark > uOffset) {
                    ...
                    // Apply the same adjustment to uRemaining from uChunkSize
                    // to allow the uRemaining -= uChunkSize below to stay
                    // correct
                    uRemaining -= uSkip;
