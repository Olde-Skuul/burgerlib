...
#if !defined(DOXYGEN)
static const D3DDECLTYPE g_D3DType[5] = {D3DDECLTYPE_FLOAT1, D3DDECLTYPE_FLOAT2,
	D3DDECLTYPE_FLOAT3, D3DDECLTYPE_FLOAT4, D3DDECLTYPE_D3DCOLOR};
	...
	if (m_uFlags & FLAG_VERTEXDIRTY) {
		...
		if (m_uVertexArraySize) {
			...
			hResult = pDevice->CreateVertexBuffer(
				static_cast<UINT>(m_uVertexArraySize), 0, 0, D3DPOOL_DEFAULT,
				&m_pD3DVertexBuffer, NULL);

			// If there's data to initialize with, upload it.

			if ((hResult >= 0) && m_pVertexArray) {
				void* pVertices = NULL;
			...
			if ((hResult >= 0) && m_uMemberCount) {
				...
				do {
					...
					uint_t uType =
						(uMember & USAGE_CHUNKMASK) >> USAGE_CHUNKMASKSHIFT;
					uOffset += g_ChunkElementSizes[uType] *
						g_ChunkElementCounts[uType];
					...
					pDest->Usage = static_cast<BYTE>(
						(uMember & USAGE_TYPEMASK) >> USAGE_TYPEMASKSHIFT);
					pDest->UsageIndex = 0;
				...
				if (hResult >= 0) {
					...
					hResult = pDevice->CreateVertexDeclaration(
						TempElement, &m_pD3DVertexDeclaration);
				}
			}
		}

		// Upload any index buffers
		if ((hResult >= 0) && m_uElementSize) {
			...
			// Create the buffer
			hResult =
				pDevice->CreateIndexBuffer(static_cast<UINT>(m_uElementSize), 0,
					uFormat, D3DPOOL_DEFAULT, &m_pD3DIndexBuffer, NULL);
			if (hResult >= 0) {
				void* pIndices = NULL;
