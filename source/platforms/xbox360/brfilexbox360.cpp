	...
	Filename* pFileName, eFileAccess uAccess) BURGER_NOEXCEPT
{
	...
	static const DWORD g_Creation[4] = {
		OPEN_EXISTING, CREATE_ALWAYS, CREATE_NEW, OPEN_ALWAYS};
	uAccess = static_cast<eFileAccess>(uAccess & 3);
	HANDLE fp = CreateFileA(pFileName->get_native(), g_Access[uAccess],
		g_Share[uAccess], NULL, g_Creation[uAccess],
