/***************************************
	...
	When running in Carbon, the registery is not visible, so this glue will
	allow the registery to be used as is it's present when using the Carbon API.

	Copyright (c) 1999-2023 by Rebecca Ann Heineman <becky@burgerbecky.com>
		...
		if (pNameRegistryLib) {
			...
			pLocals->RegistryEntryMod =
				(OSStatus(*)(RegEntryIter*, RegEntryIterationOp, RegEntryID*,
					...
					pNameRegistryLib->get_function("RegistryEntryMod");

			pLocals->RegistryPropertyGet =
				(OSStatus(*)(const RegEntryID*, const RegPropertyName*, void*,
					RegPropertyValueSize*))
					pNameRegistryLib->get_function("RegistryPropertyGet");

			pLocals->RegistryPropertyRename =
				(OSStatus(*)(const RegEntryID*, const RegPropertyName*,
					const RegPropertyName*))
					pNameRegistryLib->get_function("RegistryPropertyRename");

			pLocals->RegistryEntryIterateSet =
				(OSStatus(*)(RegEntryIter*, const RegEntryID*))
					pNameRegistryLib->get_function("RegistryEntryIterateSet");

			pLocals->RegistryEntryCopy =
				(OSStatus(*)(RegEntryID*, RegEntryID*, RegEntryID*))
					pNameRegistryLib->get_function("RegistryEntryCopy");

			pLocals->RegistryPropertyGetSize =
				(OSStatus(*)(const RegEntryID*, const RegPropertyName*,
					RegPropertyValueSize*))
					pNameRegistryLib->get_function("RegistryPropertyGetSize");

			pLocals->RegistryPropertyIterateCreate =
				(OSStatus(*)(const RegEntryID*, RegPropertyIter*))
					pNameRegistryLib->get_function(
						"RegistryPropertyIterateCreate");

			pLocals->RegistryEntryIterate =
				(OSStatus(*)(
					RegEntryIter*, RegEntryIterationOp, RegEntryID*, Boolean*))
					pNameRegistryLib->get_function("RegistryEntryIterate");
