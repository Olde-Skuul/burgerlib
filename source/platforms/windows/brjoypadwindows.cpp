...
public:
	...
	JoystickFound
		m_Joysticks[Burger::Joypad::MAXJOYSTICKS]; // Joysticks enumerated
};
...
struct JoypadRawData_t {
	...
	uint32_t m_uAxis[Burger::Joypad::MAXAXIS];
	/** POV data from the joypad (0-65535) */
	uint32_t m_uPOV[Burger::Joypad::MAXPOV];
	/** Button data (FALSE/TRUE) */
	uint8_t m_bButtons[Burger::Joypad::MAXBUTTONS];
};
...
static BOOL CALLBACK EnumJoysticksCallback(
	const DIDEVICEINSTANCEW* pdidInstance, VOID* pThis) BURGER_NOEXCEPT
{
	...
	if (!pJoystick->m_bXInputFound ||
		!Burger::IsDeviceXInput(&pdidInstance->guidProduct)) {
		...
		pFound->m_InstanceName.assign(static_cast<const uint16_t*>(
			static_cast<const void*>(pdidInstance->tszInstanceName)));
		pFound->m_ProductName.assign(static_cast<const uint16_t*>(
			static_cast<const void*>(pdidInstance->tszProductName)));
		++uIndex;
		pJoystick->m_uCount = uIndex;

		// Stop if the maximum number of devices is reached (To prevent
		// overflow)
...
struct ObjectCallBack_t {
	...
	DIOBJECTDATAFORMAT ObjectFormat[Burger::Joypad::MAXBUTTONS +
		Burger::Joypad::MAXAXIS + Burger::Joypad::MAXPOV];
	// GUID of each selected object
	GUID GUID[Burger::Joypad::MAXBUTTONS + Burger::Joypad::MAXAXIS +
		Burger::Joypad::MAXPOV];
};

/***************************************

	This function is called for each and every object in a game controller
	device It's used to count the objects and calibrate them to 0 through
	MAXAXISVALUE

***************************************/

static BOOL CALLBACK EnumObjectsCallback(
	const DIDEVICEOBJECTINSTANCEW* pObject, VOID* pThis) BURGER_NOEXCEPT
{
	...
	if (pObject->dwType & DIDFT_BUTTON) {
		if (pCallback->m_uButtonCount < Burger::Joypad::MAXBUTTONS) {
			pCurrent->dwOfs = BURGER_OFFSETOF(JoypadRawData_t, m_bButtons) +
		...
		if (pCallback->m_uAxisCount < Burger::Joypad::MAXAXIS) {
			...
			PropertyRange.diph.dwObj =
				pObject->dwType; // Specify the enumerated axis
			PropertyRange.lMin = 0;
			PropertyRange.lMax = Burger::Joypad::MAXAXISVALUE;
			// Set the range for the axis
			if (pCallback->m_pDevice->SetProperty(
					DIPROP_RANGE, &PropertyRange.diph) < 0) {
				return DIENUM_STOP;
			}
			pCurrent->dwOfs = BURGER_OFFSETOF(JoypadRawData_t, m_uAxis) +
				(pCallback->m_uAxisCount * sizeof(uint32_t));
		...
		if (pCallback->m_uPOVCount < Burger::Joypad::MAXPOV) {
			pCurrent->dwOfs = BURGER_OFFSETOF(JoypadRawData_t, m_uPOV) +
				(pCallback->m_uPOVCount * sizeof(uint32_t));
	  ...
	  m_uDeviceCount(0)
{
	...
	if (Win32::load_function(Win32::kCall_XInputGetState)) {
		...
		do {
			...
			pJoypadData->m_InstanceName.assign(
				"Gamepad for Xbox 360 (Controller)");
			pJoypadData->m_ProductName.assign(
				"Gamepad for Xbox 360 (Controller)");
			// pJoypadData->m_bConnected = FALSE;
			++pJoypadData;
		} while (--uXInputCount);
	...
	if (pDirectInput8W) {
		...
		//
		// Enumerate the devices, after this function, the number of gaming
		// devices and their GUIDs are known
		//
		...
		HRESULT hResult = pDirectInput8W->EnumDevices(DI8DEVCLASS_GAMECTRL,
			EnumJoysticksCallback, &EnumeratedJoysticks, DIEDFL_ALLDEVICES);
		...
		if ((hResult >= 0) && EnumeratedJoysticks.m_uCount) {
			...
			do {
				...
				hResult = pDirectInput8W->CreateDevice(
					pFound->m_InstanceGUID, &pJoystickDeviceLocal, NULL);
				if (hResult >= 0) {
					IDirectInputDevice8W* pJoystickDevice =
					...
					hResult = pJoystickDevice->SetCooperativeLevel(
						pAppInstance->GetWindow(),
						DISCL_FOREGROUND | DISCL_EXCLUSIVE);
					if (hResult >= 0) {
						//
						// Begin the scan for the input objects found in this
						// device
						//
						...
						Object.Format.dwObjSize =
							sizeof(Object.ObjectFormat[0]);
						...
						hResult = pJoystickDevice->EnumObjects(
							EnumObjectsCallback, &Object, DIDFT_ALL);
						if (hResult >= 0) {
							hResult =
								pJoystickDevice->SetDataFormat(&Object.Format);
							if (hResult >= 0) {
								// Accept this device!
								pJoypadData->m_uButtonCount =
									Object.m_uButtonCount;
	...
	void* pData) BURGER_NOEXCEPT
{
	...
	if (pThis->m_bXInputFound) {
		...
		do {
			...
			if (bIsConnected) {
				uint32_t uButtons;
				const uint_t uXBoxButtons = State.Gamepad.wButtons;
	...
	(XINPUT_GAMEPAD_DPAD_UP == 1) && (XINPUT_GAMEPAD_DPAD_DOWN == 2)
				...
				uButtons += ((uXBoxButtons &
					...
					<< 14U); // Bits 20-21
				uButtons += ((uXBoxButtons & (XINPUT_GAMEPAD_BIGBUTTON))
					<< 11); // Bit 22
#else
				uButtons = 0;
		...
		do {
			...
			if (pJoystickDevice) {
				...
				if (hResult < 0) {
					...
					hResult =
						pJoystickDevice->GetDeviceState(sizeof(State), &State);
					if (hResult >= 0) {
						...
						if (uButtonCount) {
							...
							/*	Window returns the position of the hi-hat (POV)
							   as -1 (JOY_POVCENTERED) or a value between 0 (0
							   degrees) and 36000 (360 degrees). 0 is forward,
							   9000 is right, 18000 is backward, and 27000 is
							   left. Rather than saying the hat is pressed to
							   the right only when the Windows POV value is
							   exactly 9000 (90 degrees), we will say anything
							   between 15 degrees (forward plus some slop) and
							   165 degrees (backward minus some slop) means the
							   hat is to the right.
							*/
							...
							if (uTemp != INVALIDPOVVALUE) {
								if ((uTemp > POVANALOGLEFT + POV_SLOP &&
										uTemp <= MAXPOVVALUE) ||
									(uTemp < POVANALOGRIGHT -
											POV_SLOP)) { /* we will consider the
															hat to be up if it
															is anywhere forward
															of left or right */
									uButtons += POVUP;
								}
								if ((uTemp > POVANALOGFORWARD + POV_SLOP) &&
									(uTemp < POVANALOGBACKWARD -
											POV_SLOP)) { /* we will consider the
															hat to be to the
															right up if it is
															anywhere to the
								...
								if ((uTemp > POVANALOGRIGHT + POV_SLOP) &&
									(uTemp < POVANALOGLEFT - POV_SLOP)) {
									uButtons += POVDOWN;
								}
								if ((uTemp > POVANALOGBACKWARD + POV_SLOP) &&
									(uTemp < MAXPOVVALUE - POV_SLOP)) {
									uButtons += POVLEFT;
								}
							}
							// Convert analog directions to digital info
...
/*! ************************************
	...
	When the application gains focus, call this function to acquire the
	joysticks
...
/*! ************************************
	...
	When the application loses focus, call this function to release the
	joysticks
...
uint_t BURGER_API Burger::XInputGetGamepadState(uint_t uWhich,
	...
	eXInputDeadZoneType uDeadZoneType) BURGER_NOEXCEPT
{
	uint32_t uResult;
	if ((uWhich >= 4) || !pXInputGamePad) {
		...
		if (uResult != ERROR_CALL_NOT_IMPLEMENTED) {
			...
			// Get the old and new states
			const uint_t bWasConnected =
				pXInputGamePad->m_bConnected != 0; // Force boolean for & below
			const uint_t bIsConnected = (uResult == ERROR_SUCCESS);
			...
			if (bIsConnected) {
				...
				const uint32_t bButtons = GamepadState.Gamepad.wButtons;
...
/*! ************************************
	...
	\return \ref FALSE if the device is not supported by XInput, \ref TRUE if
		so.

***************************************/

uint_t BURGER_API Burger::IsDeviceXInput(const GUID* pGuid) BURGER_NOEXCEPT
{
	...
	if (Win32::is_vista_or_higher()) {
		...
		if ((CoCreateInstance(CLSID_WbemLocator, NULL, CLSCTX_INPROC_SERVER,
				 IID_IWbemLocator, (LPVOID*)&pIWbemLocator) >= 0) &&
			pIWbemLocator) {

			// Connect to WMI
			IWbemServices* pIWbemServices = NULL;
			if ((pIWbemLocator->ConnectServer(
					 (const BSTR)(L"\\\\.\\root\\cimv2"), NULL, NULL, NULL, 0,
					 NULL, NULL, &pIWbemServices) >= 0) &&
				pIWbemServices) {

				// Switch security level to IMPERSONATE.
				CoSetProxyBlanket(pIWbemServices, RPC_C_AUTHN_WINNT,
					RPC_C_AUTHZ_NONE, NULL, RPC_C_AUTHN_LEVEL_CALL,
					RPC_C_IMP_LEVEL_IMPERSONATE, NULL, EOAC_NONE);

				// Get the PNPEntity list
				IEnumWbemClassObject* pEnumDevices = NULL;
				if ((pIWbemServices->CreateInstanceEnum(
						 (const BSTR)(L"Win32_PNPEntity"), 0, NULL,
						 &pEnumDevices) >= 0) &&
					pEnumDevices) {
					...
					do {
						...
						do {
							...
							VARIANT MyVariant;
							if ((DevicePointers[uDevice]->Get(
									 (const BSTR)(L"DeviceID"), 0, &MyVariant,
									 NULL, NULL) >= 0) &&
								(MyVariant.vt == VT_BSTR)) {

								const uint16_t* pVariantName =
									reinterpret_cast<const uint16_t*>(
										MyVariant.bstrVal);
								if (pVariantName) {
									// Check if the device ID contains "IG_". If
									// it does, then it's an XInput device This
									// information can not be found from
									// DirectInput

									if (StringString(pVariantName,
											(const uint16_t*)L"IG_")) {

										// If it does, then get the VID/PID from
										// var.bstrVal
										uint32_t uFoundVendorID = 0;
										const uint16_t* pFound =
											StringString(pVariantName,
												(const uint16_t*)L"VID_");
										if (pFound) {
											uFoundVendorID = AsciiHexToInteger(
												pFound + 4, 4);
										}
										uint32_t uFoundPeripheralID = 0;
										pFound = StringString(pVariantName,
											(const uint16_t*)L"PID_");
										if (pFound) {
											uFoundPeripheralID =
												AsciiHexToInteger(
													pFound + 4, 4);
										...
										// Check the Peripheral ID to the one in
										// the GUID
										const uint32_t uVIDPID =
											(uFoundVendorID & 0xFFFF) +
											(uFoundPeripheralID << 16);
							...
							if (DevicePointers[uDevice]) {
								...
								DevicePointers[uDevice] = nullptr;
							}
		...
		// Note to self, don't EVER trust sample code from a Microsoft web site.
		// They will LIE to you about whether a piece of code (See above) would
		...
		if (!GetRawInputDeviceList(
				NULL, &uDeviceCount, sizeof(RAWINPUTDEVICELIST)) &&
			uDeviceCount) {
						...
						if (pWorkList->dwType == RIM_TYPEHID) {
							...
							if (GetRawInputDeviceInfoW(pWorkList->hDevice,
								...
								if ((RawDeviceInfo.hid.dwVendorId ==
										(pGuid->Data1 & 0xFFFF)) &&
									(RawDeviceInfo.hid.dwProductId ==
										(pGuid->Data1 >> 16))) {

									// Now comes the moment of truth. Get the
									// driver's name
									...
									// Exit the loop now, because the device was
									// found.
									break;
								}
							}
