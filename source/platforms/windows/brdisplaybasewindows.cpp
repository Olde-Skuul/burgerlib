...
{
	Burger::Display::VideoCardDescription* pOutput =
		static_cast<Burger::Display::VideoCardDescription*>(pInput);
	...
	if (pSurface->dwFlags & (DDSD_HEIGHT | DDSD_WIDTH | DDSD_PIXELFORMAT)) {
		...
		if (pSurface->dwFlags & DDSD_REFRESHRATE) {
			...
			Entry.m_uHertz =
				0; // Force to zero as an alternate way of clearing the rate
		}
...
static HRESULT __stdcall EnumerateVideoDevice(GUID* pGUID, char* pDescription,
	char* pName, void* pInput, HMONITOR pMonitor)
{
	// Ignore NULL GUID (Primary)
	if (pGUID) {
		Burger::ClassArray<Burger::Display::VideoCardDescription>* pOutput =
			static_cast<
				Burger::ClassArray<Burger::Display::VideoCardDescription>*>(
				pInput);
		...
		if (Burger::Win32::DirectDrawCreateEx(pGUID,
			...
			Entry.m_DeviceName =
				pDescription; // Copy the name of the video card
			...
			if (GetMonitorInfoW(pMonitor, &MonitorInfo)) {
				...
				Entry.m_CurrentResolution.SetRight(static_cast<int>(
					MonitorInfo.rcMonitor.right - MonitorInfo.rcMonitor.left));
				Entry.m_CurrentResolution.SetBottom(static_cast<int>(
					MonitorInfo.rcMonitor.bottom - MonitorInfo.rcMonitor.top));

				// Is this the primary monitor?
				if (MonitorInfo.dwFlags & MONITORINFOF_PRIMARY) {
					Entry.m_uFlags |= Burger::Display::VideoCardDescription::
						VIDEOCARD_PRIMARY;
				}
			}
			...
			if (pDirectDraw7->EnumDisplayModes(
					DDEDM_REFRESHRATES, 0, &Entry, ModeCallBack) != DD_OK) {
				// If refresh rates were not permitted, try again without them
				if (pDirectDraw7->EnumDisplayModes(
						0, 0, &Entry, ModeCallBack) != DD_OK) { // Oh oh...
					bFailed = TRUE;
				}
			}
...
void BURGER_API Burger::Display::InitGlobals(void)
{
	if (!g_Globals.m_bInitialized) {
		...
		g_Globals.m_uDefaultWidth =
			static_cast<uint_t>(GetDeviceCaps(hDC, HORZRES));
		g_Globals.m_uDefaultHeight =
			static_cast<uint_t>(GetDeviceCaps(hDC, VERTRES));

		// Get the bit depth
		g_Globals.m_uDefaultDepth = static_cast<uint_t>(
			GetDeviceCaps(hDC, PLANES) * GetDeviceCaps(hDC, BITSPIXEL));

		g_Globals.m_uDefaultHertz =
			static_cast<uint_t>(GetDeviceCaps(hDC, VREFRESH));

		g_Globals.m_uDefaultTotalWidth =
			static_cast<uint_t>(GetSystemMetrics(SM_CXVIRTUALSCREEN) -
				GetSystemMetrics(SM_XVIRTUALSCREEN));
		g_Globals.m_uDefaultTotalHeight =
			static_cast<uint_t>(GetSystemMetrics(SM_CYVIRTUALSCREEN) -
				GetSystemMetrics(SM_YVIRTUALSCREEN));
		g_Globals.m_uDefaultMonitorCount =
			static_cast<uint_t>(GetSystemMetrics(SM_CMONITORS));
...
/*! ************************************
	...
	To handle window resizing, this function is called from the Burgerlib
	windows callback when WM_GETMINMAXINFO messages are passed. This function
	determines if resizing should be disabled or limited to specific sizes or
	aspect ratios

	\param pWindow HWND of the window the callback was handling
	\param lParam lParam Value passed from Windows to the callback which is a
		pointer to a MINMAXINFO structure

	\return \ref FALSE if this event is ignored, or \ref TRUE if this event was
		intercepted

***************************************/

uint_t BURGER_API Burger::Display::HandleMinMax(
	HWND__* pWindow, uintptr_t lParam)
{
	uint_t bResult = FALSE;
	// If full screen or not allowed, disable
	const uint_t uFlags = m_uFlags;

	// Check if this resizing should be disabled
	if ((uFlags & FULLSCREEN) || !(uFlags & ALLOWRESIZING)) {

		// This code will lock out all resizing events and force the window back
		// to the size of the game screen

		const LONG lScreenWidth = static_cast<LONG>(m_uWidth);
		const LONG lScreenHeight = static_cast<LONG>(m_uHeight);
		if (lScreenWidth && lScreenHeight) {
			// Adjust the window size to whatever the video manager says it
			// should be
			RECT WindowSizeRect = {0, 0, lScreenWidth, lScreenHeight};
			AdjustWindowRectEx(&WindowSizeRect,
				static_cast<DWORD>(GetWindowLongPtrW(pWindow, GWL_STYLE)),
				GetMenu(pWindow) != 0,
				static_cast<DWORD>(GetWindowLongPtrW(pWindow, GWL_EXSTYLE)));

			// Set the minimum and maximum window sizes to the same value to
			// perform the resize disabling
