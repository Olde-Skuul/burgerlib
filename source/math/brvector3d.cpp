...
/*! ************************************
    ...
    set of common functions for simple 3 dimensional math are part of the
    structure.

    It is expected to be 4 byte aligned and use scalar math. The members are
    hard coded to be "x", "y" and "z" for maximum compatibility
...
/*! ************************************
    ...
    \sa Zero(void), SetX(float), SetY(float), SetZ(float) or Set(const
        Vector3D_t *)
...
void BURGER_API Burger::Vector3D_t::Interpolate(const Vector3D_t* pFrom,
    const Vector3D_t* pTo, float fFactor) BURGER_NOEXCEPT
{
    const float fFromX = pFrom->x;
    const float fFromY = pFrom->y;
    const float fFromZ = pFrom->z;
...
/*! ************************************
    ...
    \brief Return a dot product of two 3D vectors
    \param pInput Valid pointer to a Vector3D_t structure to perform a dot
        product against \return Dot product of the two vectors
...
BURGER_DECLSPECNAKED float BURGER_API Burger::Vector3D_t::Dot(const Vector3D_t *pInput) const BURGER_NOEXCEPT
{
	BURGER_ASM {
	fld		dword ptr [ecx+0]
...
/*! ************************************
    ...
    \sa Add(float), Add(const Vector3D_t *,const Vector3D_t *) or Add(const
        Vector3D_t *,float)
...
/*! ************************************
    ...
    \sa Add(const Vector3D_t *), Add(const Vector3D_t *,const Vector3D_t *) or
        Add(const Vector3D_t *,float)

***************************************/

/*! ************************************

    \fn void Burger::Vector3D_t::Add(const Vector3D_t *pInput1,
...
/*! ************************************
    ...
    \sa Add(float), Add(const Vector3D_t *) or Add(const Vector3D_t *,const
        Vector3D_t *)
...
/*! ************************************
    ...
    \sa Sub(float), Sub(const Vector3D_t *,const Vector3D_t *) or Sub(const
        Vector3D_t *,float)
...
/*! ************************************
    ...
    \sa Sub(const Vector3D_t *), Sub(const Vector3D_t *,const Vector3D_t *) or
        Sub(const Vector3D_t *,float)

***************************************/

/*! ************************************

    \fn void Burger::Vector3D_t::Sub(const Vector3D_t *pInput1,
        const Vector3D_t *pInput2)
...
/*! ************************************
    ...
    \brief Subract a vector and a scalar to every member of the vector and store
    ...
    \sa Sub(float), Sub(const Vector3D_t *) or Sub(const Vector3D_t *,const
        Vector3D_t *)
...
/*! ************************************
    ...
    \sa Mul(float), Mul(const Vector3D_t *,const Vector3D_t *) or Mul(const
        Vector3D_t *,float)
...
/*! ************************************
    ...
    \sa Mul(const Vector3D_t *), Mul(const Vector3D_t *,const Vector3D_t *) or
        Mul(const Vector3D_t *,float)

***************************************/

/*! ************************************

    \fn void Burger::Vector3D_t::Mul(const Vector3D_t *pInput1,
        const Vector3D_t *pInput2)
...
/*! ************************************
    ...
    \sa Mul(float), Mul(const Vector3D_t *) or Mul(const Vector3D_t *,const
        Vector3D_t *)
...
/*! ************************************
    ...
    \fn void Burger::Vector3D_t::MulAdd(const Vector3D_t *pMul,
        const Vector3D_t *pAdd)
    \brief Multiply two vectors and add a third and store the result in this
        vector
    ...
    \sa MulAdd(float,const Vector3D_t *), MulAdd(const Vector3D_t *,const
        Vector3D_t *,const Vector3D_t *) or MulAdd(const Vector3D_t
        *,float,const Vector3D_t *)

***************************************/

/*! ************************************

    \fn void Burger::Vector3D_t::MulAdd(const Vector3D_t *pMul1,
        const Vector3D_t *pMul2,const Vector3D_t *pAdd)
    \brief Multiply two vectors and add a third and store the result in this
        vector
    ...
    \param pAdd Vector to add
    \sa MulAdd(float,const Vector3D_t *), MulAdd(const Vector3D_t *,const
        Vector3D_t *) or MulAdd(const Vector3D_t *,float,const Vector3D_t *)
...
/*! ************************************
    ...
    \brief Multiply this vector by a scalar and add a second vector and store
        the result in this vector

    Initialize this vector with the result of the multiplication of a vector and
    a scalar and add the vector pAdd
    ...
    \sa MulAdd(const Vector3D_t *,const Vector3D_t *,const Vector3D_t *),
        MulAdd(const Vector3D_t *,const Vector3D_t *) or MulAdd(const Vector3D_t
        *,float,const Vector3D_t *)
...
/*! ************************************
    ...
    \fn void Burger::Vector3D_t::MulAdd(const Vector3D_t *pMul,float
        fInput,const Vector3D_t *pAdd)
    \brief Multiply a vector by a scalar and add a second vector and store the
        result in this vector
...
void BURGER_API Burger::Vector3D_t::Cross(
    const Vector3D_t* pInput) BURGER_NOEXCEPT
{
    const float fX = (y * pInput->z) - (z * pInput->y);
    const float fY = (z * pInput->x) - (x * pInput->z);
    const float fZ = (x * pInput->y) - (y * pInput->x);
    x = fX;
    y = fY;
    z = fZ;
}
...
/*! ************************************
    ...
    Calculate the cross product between two 3D vectors. The end result is the
    perpendicular vector stored in this Vector3D_t instance.

    x | y | z
    :--:|:--:|:--:
    (pInput1->y * pInput2->z) - (pInput1->z * pInput2->y) | (pInput1->z * pInput2->x) - (pInput1->x * pInput2->z) | (pInput1->x * pInput2->y) - (pInput1->y * pInput2->x)

    \param pInput1 Valid pointer to a Vector3D_t structure to perform a cross
        product from
...
{
    const float fX = (pInput1->y * pInput2->z) - (pInput1->z * pInput2->y);
    const float fY = (pInput1->z * pInput2->x) - (pInput1->x * pInput2->z);
    const float fZ = (pInput1->x * pInput2->y) - (pInput1->y * pInput2->x);
    x = fX;
    y = fY;
    z = fZ;
}
...
// defined(BURGER_METROWERKS) || defined(BURGER_MSVC))
BURGER_DECLSPECNAKED void Burger::Vector3D_t::Cross(
    const Vector3D_t* pInput1, const Vector3D_t* pInput2) BURGER_NOEXCEPT
{
    // clang-format off
    BURGER_ASM
...
#if 0
BURGER_DECLSPECNAKED float Burger::Vector3D_t::GetLengthSquared(
...
float BURGER_API Burger::Vector3D_t::GetLengthFast(void) const
{
    ...
    // Valid number
    if (n) { 
        uint32_t num = ((uint32_t*)&n)[0];
        num = (num >> (23 - TABLEBITS)) &
            (0xFFFFFF >> (23 - TABLEBITS)); // leave only the mantissa
        uint32_t Result =
            (BurgerSqrtTable[num] + (((uint32_t*)&n)[0] & 0x7F000000)) >> 1;
...
/*! ************************************
    ...
    Create a vector from the difference of two 3D points and return the vector's
    length squared
    ...
    \sa GetDistanceSquared(const Vector3D_t *) const,
        GetDistance(float,float,float) const or
        GetDistanceFast(float,float,float) const

***************************************/

float BURGER_API Burger::Vector3D_t::GetDistanceSquared(
...
/*! ************************************
    ...
    \param pInput Valid pointer to a Vector3D_t structure that contains the
        other 3D point
    \return The distance between the two points, squared
    \sa GetDistanceSquared(float,float,float) const, GetDistance(const
        Vector3D_t *) const or GetDistanceFast(const Vector3D_t *) const
...
/*! ************************************
    ...
    \sa GetDistance(const Vector3D_t *) const,
        GetDistanceSquared(float,float,float) const or
        GetDistanceFast(float,float,float) const

***************************************/

float BURGER_API Burger::Vector3D_t::GetDistance(
    float fX, float fY, float fZ) const BURGER_NOEXCEPT
{
    fX = x - fX;
    fY = y - fY;
...
/*! ************************************
    ...
    \param pInput Valid pointer to a Vector3D_t structure that contains the
        other 3D point
    \return The distance between the two points
    \sa GetDistance(float,float,float) const, GetDistanceSquared(const
        Vector3D_t *) const or GetDistanceFast(const Vector3D_t *) const
...
/*! ************************************
    ...
    \brief Returns the square of the distance between two 3D points (Good
        precision).
    ...
    \param fZ Z value of the other 3D point
    \return The distance between the two points
    \sa GetDistanceFast(const Vector3D_t *) const,
        GetDistanceSquared(float,float,float) const or
        GetDistance(float,float,float) const

***************************************/

float BURGER_API Burger::Vector3D_t::GetDistanceFast(
    float fX, float fY, float fZ) const BURGER_NOEXCEPT
{
    fX = x - fX;
    fY = y - fY;
...
/*! ************************************
    ...
    \param pInput Valid pointer to a Vector3D_t structure that contains the
        other 3D point
    \return The distance between the two points
    \sa GetDistanceFast(float,float,float) const,
        GetDistanceSquared(const Vector3D_t *) const or
        GetDistance(const Vector3D_t *) const
...
float BURGER_API Burger::Vector3D_t::GetDistanceFast(
    const Vector3D_t* pInput) const BURGER_NOEXCEPT
{
    float x = Input1->x - Input2->x;
    float y = Input1->y - Input2->y;
    float z = Input1->z - Input2->z;
    float n = ((x * x) + (y * y) + (z * z));
    if (n) { /* Valid number */
        uint32_t num = ((uint32_t*)&n)[0];
        num = (num >> (23 - TABLEBITS)) &
...
/*! ************************************
    ...
    \sa Normalize(float,float,float), Normalize(const Vector3D_t *) or
        NormalizeFast(void)

***************************************/

void BURGER_API Burger::Vector3D_t::Normalize(void) BURGER_NOEXCEPT
{
...
/*! ************************************
    ...
    \sa Normalize(void), Normalize(const Vector3D_t *) or
        NormalizeFast(float,float,float)

***************************************/

void BURGER_API Burger::Vector3D_t::Normalize(
...
/*! ************************************
    ...
    \param pInput Valid pointer to a Vector3D_t structure that will be copied
        and normalize
    \sa Normalize(void), Normalize(float,float,float) or
        NormalizeFast(const Vector3D_t *)
...
#if 0
BURGER_DECLSPECNAKED void BURGER_API Burger::Vector3D_t::Normalize(
    const Vector3D_t* pInput) BURGER_NOEXCEPT
{
    // clang-format off
	BURGER_ASM {
...
{
    const float fVect0x = pOrigin->x - pPoint1->x; // First difference
    const float fVect0y = pOrigin->y - pPoint1->y;
    const float fVect0z = pOrigin->z - pPoint1->z;

    const float fVect1x = pOrigin->x - pPoint2->x; // Second difference
    const float fVect1y = pOrigin->y - pPoint2->y;
    const float fVect1z = pOrigin->z - pPoint2->z;
...
/*! ************************************
    ...
    \sa NormalizeFast(float,float,float), NormalizeFast(const Vector3D_t *) or
        Normalize(void)

***************************************/

void BURGER_API Burger::Vector3D_t::NormalizeFast(void) BURGER_NOEXCEPT
{
    float fLengthSquared = (x * x) + (y * y) + (z * z);
    if (fLengthSquared > 0.0f) {                      // Handles NaN
        fLengthSquared = 1.0f / square_root(fLengthSquared); // Reciprocal
...
/*! ************************************
    ...
    \param fY Y value of the 3D vector to normalize
    \param fZ Z value of the 3D vector to normalize
    \sa NormalizeFast(void), NormalizeFast(const Vector3D_t *) or
        Normalize(float,float,float)

***************************************/

void BURGER_API Burger::Vector3D_t::NormalizeFast(
    float fX, float fY, float fZ) BURGER_NOEXCEPT
{
    float fLengthSquared = (fX * fX) + (fY * fY) + (fZ * fZ);
    if (fLengthSquared > 0.0f) {                      // Handles NaN
...
/*! ************************************
    ...
    \param pInput Valid pointer to a Vector3D_t structure that will be copied
        and normalize
    \sa NormalizeFast(void), NormalizeFast(float,float,float) or
        Normalize(const Vector3D_t *)
...
/*! ************************************
    ...
    \note This performs a binary comparison, so NaN forced inequality will not
        work. Use Vector3D_t::Equal(const Vector3D_t *,float) const instead
    ...
    const Vector3D_t* pInput) const BURGER_NOEXCEPT
{
    ...
    const uint32_t uTemp =
        ((static_cast<const uint32_float_t*>(static_cast<const void*>(&x))->w ^
            ...
            (static_cast<const uint32_float_t*>(static_cast<const void*>(&z))->w ^
                static_cast<const uint32_float_t*>(
                    static_cast<const void*>(&pInput->z))->w));
    return (uTemp == 0);
}
...
/*! ************************************
    ...
    Perform a difference of each member and if they are less than a specific
    range, return \ref TRUE for equality

    \param pInput Vector to compare against
    \param fRange Epsilon range to use for equality
    \return \ref TRUE if the vectors are close enough to be equal or \ref FALSE
        if not.
...
/*! ************************************
    ...
    \brief Compare two Vector3D_t's for bitwise equality

    Unlike operator==(const Vector3D_t &) const, this function performs
    a bitwise comparison, which in some cases is faster if pure
    equality detection is desired.

    \param pInput Pointer to the Vector3D_t to compare against
    \return \ref TRUE if equal, \ref FALSE if not
    \sa operator==(const Vector3D_t&) const

***************************************/

uint_t BURGER_API Burger::Vector3D_t::BitwiseEqual(
    const Vector3D_t* pInput) const BURGER_NOEXCEPT
{
    return (
        (static_cast<const uint32_float_t*>(
...
/*! ************************************
    ...
    \fn float & Burger::Vector3D_t::operator[](uintptr_t uInput)
...
/*! ************************************
    ...
    \fn const float & Burger::Vector3D_t::operator[](uintptr_t uInput) const
    \brief Access the members as an array
...
/*! ************************************
    ...
    \fn Burger::Vector3D_t::operator == (const Vector3D_t& rInput) const
...
/*! ************************************
    ...
    This convenience operator converts the Vector3D_t into a float pointer to
...
/*! ************************************
    ...
    \brief Structure to create a Vector3D_t with 32 bit unsigned integer
        constants
    ...
    union between the Vector3D_t structure and an array of data that maps
    directly over it.
