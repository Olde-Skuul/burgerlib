...
/*! ************************************
	...
	This 8 byte vector contains x and y 32 bit floating point coordinates. A
...
void BURGER_API Burger::Vector2D_t::Interpolate(const Vector2D_t *pFrom,const Vector2D_t *pTo,float fFactor)
{
...
/*! ************************************
	...
	\brief Return a dot product of two 2D vectors
	\param pInput Valid pointer to a Vector2D_t structure to perform a dot product against
	\return Dot product of the two vectors
	\sa Dot(float,float) const
...
/*! ************************************
	...
	\sa Add(float), Add(const Vector2D_t *,const Vector2D_t *) or Add(const Vector2D_t *,float)
...
/*! ************************************
	...
	\sa Add(const Vector2D_t *), Add(const Vector2D_t *,const Vector2D_t *) or Add(const Vector2D_t *,float)

***************************************/

/*! ************************************

	\fn void Burger::Vector2D_t::Add(const Vector2D_t *pInput1,const Vector2D_t *pInput2)
...
/*! ************************************
	...
	\sa Add(float), Add(const Vector2D_t *) or Add(const Vector2D_t *,const Vector2D_t *)
...
/*! ************************************
	...
	\sa Sub(float), Sub(const Vector2D_t *,const Vector2D_t *) or Sub(const Vector2D_t *,float)
...
/*! ************************************
	...
	\sa Sub(const Vector2D_t *), Sub(const Vector2D_t *,const Vector2D_t *) or Sub(const Vector2D_t *,float)

***************************************/

/*! ************************************

	\fn void Burger::Vector2D_t::Sub(const Vector2D_t *pInput1,const Vector2D_t *pInput2)
...
/*! ************************************
	...
	\sa Sub(float), Sub(const Vector2D_t *) or Sub(const Vector2D_t *,const Vector2D_t *)
...
/*! ************************************
	...
	\sa Mul(float), Mul(const Vector2D_t *,const Vector2D_t *) or Mul(const Vector2D_t *,float)
...
/*! ************************************
	...
	\sa Mul(const Vector2D_t *), Mul(const Vector2D_t *,const Vector2D_t *) or Mul(const Vector2D_t *,float)

***************************************/

/*! ************************************

	\fn void Burger::Vector2D_t::Mul(const Vector2D_t *pInput1,const Vector2D_t *pInput2)
...
/*! ************************************
	...
	\sa Mul(float), Mul(const Vector2D_t *) or Mul(const Vector2D_t *,const Vector2D_t *)
...
/*! ************************************
	...
	Calculate the cross product between two 2D vectors as if they were
	two 3D vectors and the z components were zero. The end result is
	the new z component for the perpendicular vector
...
/*! ************************************
	...
	\param pInput Valid pointer to a Vector2D_t structure to perform a cross product against
	\return Cross product of the two vectors
...
/*! ************************************
	...
	Create a vector from the difference of two 2D points and return
	...
	\sa GetDistanceSquared(const Vector2D_t *) const, GetDistance(float,float) const or GetDistanceFast(float,float) const

***************************************/

float BURGER_API Burger::Vector2D_t::GetDistanceSquared(float fX,float fY) const
{
...
/*! ************************************
	...
	\param pInput Valid pointer to a Vector2D_t structure that contains the other 2D point
	\return The distance between the two points, squared
	\sa GetDistanceSquared(float,float) const, GetDistance(const Vector2D_t *) const or GetDistanceFast(const Vector2D_t *) const
...
/*! ************************************
	...
	\sa GetDistance(const Vector2D_t *) const, GetDistanceSquared(float,float) const or GetDistanceFast(float,float) const

***************************************/

float BURGER_API Burger::Vector2D_t::GetDistance(float fX,float fY) const
{
	fX = x-fX;
	fY = y-fY;
...
/*! ************************************
	...
	\param pInput Valid pointer to a Vector2D_t structure that contains the other 2D point
	\return The distance between the two points
	\sa GetDistance(float,float) const, GetDistanceSquared(const Vector2D_t *) const or GetDistanceFast(const Vector2D_t *) const
...
/*! ************************************
	...
	\brief Returns the square of the distance between two 2D points (Good precision)
	...
	\param fY Y value of the other 2D point
	\return The distance between the two points
	\sa GetDistanceFast(const Vector2D_t *) const, GetDistanceSquared(float,float) const or GetDistance(float,float) const

***************************************/

float BURGER_API Burger::Vector2D_t::GetDistanceFast(float fX,float fY) const
{
	fX = x-fX;
	fY = y-fY;
	return Sqrt((fX*fX)+(fY*fY));
...
/*! ************************************
	...
	\param pInput Valid pointer to a Vector2D_t structure that contains the other 2D point
	\return The distance between the two points
	\sa GetDistanceFast(float,float) const, GetDistanceSquared(const Vector2D_t *) const or GetDistance(const Vector2D_t *) const
...
/*! ************************************
	...
	\sa Normalize(float,float), Normalize(const Vector2D_t *) or NormalizeFast(void)
...
/*! ************************************
	...
	Make a copy of a 2D vector and set the copy's
	length to 1.0f

	\param fX X value of the 2D vector to normalize
	\param fY Y value of the 2D vector to normalize
	\sa Normalize(void), Normalize(const Vector2D_t *) or NormalizeFast(float,float)

***************************************/

void BURGER_API Burger::Vector2D_t::Normalize(float fX,float fY)
{
...
/*! ************************************
	...
	\param pInput Valid pointer to a Vector2D_t structure that will be copied and normalize
	\sa Normalize(void), Normalize(float,float) or NormalizeFast(const Vector2D_t *)
...
/*! ************************************
	...
	\sa NormalizeFast(float,float), NormalizeFast(const Vector2D_t *) or Normalize(void)
...
/*! ************************************
	...
	\param fX X value of the 2D vector to normalize
	\param fY Y value of the 2D vector to normalize
	\sa NormalizeFast(void), NormalizeFast(const Vector2D_t *) or Normalize(float,float)

***************************************/

void BURGER_API Burger::Vector2D_t::NormalizeFast(float fX,float fY)
{
	float fLengthSquared = (fX*fX) + (fY*fY);
	if (fLengthSquared > 0.0f) {	// Handles NaN
		fLengthSquared = 1.0f / Sqrt(fLengthSquared);	// Reciprocal
...
/*! ************************************
	...
	\param pInput Valid pointer to a Vector2D_t structure that will be copied and normalize
	\sa NormalizeFast(void), NormalizeFast(float,float) or Normalize(const Vector2D_t *)
...
/*! ************************************
	...
	\fn float Burger::Vector2D_t::Determinant(const Vector2D_t* pInput1,const Vector2D_t *pInput2) const
	\brief Calculate the determinant between three points

	Using the formula ((pInput1->x-x)*(pInput2->y-y))-((pInput1->y-y)*(pInput2->x-x)),
	calculate the determinant and return the result. Used for determining
	if a point is on a side of a vector or on the vector.
	...
	\return Zero if they are identical directions, negative for one side, positive for the other side.
...
/*! ************************************
	...
	Test the point against the three arcs generated by the three
	triangle points and if the point is within all of the arcs,
	return \ref TRUE since the point is inside the triangle.

	\note This function will also return \ref TRUE if the point
		is on the triangle edges.
...
Word BURGER_API Burger::Vector2D_t::IsPointInTriangle(const Vector2D_t *pVertex1,const Vector2D_t *pVertex2,const Vector2D_t *pVertex3) const
{
	Word uResult = FALSE;
...
/*! ************************************
	...
	\fn const float & Burger::Vector2D_t::operator[](Word uInput) const
	\brief Access the members as an array
...
/*! ************************************
	...
	This convenience operator converts the Vector2D_t into 
