    ...
    if (uTemp) { // Can I start the loop?
        do {
            ...
            if (uTemp < 0x80U) {
                ...
                // The MacRomanUS table only has 2 or 3 byte tokens
                const uint8_t* pSrc = MacRomanUS::ToUTF8Table[uTemp - 0x80U];
                if (pWorkPtr < pEndPtr) {
    ...
    if (Temp) {
        do {
            ...
            if (Temp < 0x80U) {
                ...
                // The Win1252 table only has 2 or 3 byte tokens
                const uint8_t* pSrc = Win1252::ToUTF8Table[Temp - 0x80U];
                if (pWorkPtr < pEndPtr) {
                    // Write the first character
                    pWorkPtr[0] = pSrc[0];
                }
    ...
    if (uInputSize) {
        do {
            ...
            if (uTemp < 0x80U) {
                ...
                // The Win1252 table only has 2 or 3 byte tokens
                const uint8_t* pSrc = Win1252::ToUTF8Table[uTemp - 0x80U];
                if (pWorkPtr < pEndPtr) {
                    // Write the first character
                    pWorkPtr[0] = pSrc[0];
                }
    ...
    if (uTemp) {
        do {
            ...
            if (uTemp < 0x80U) {
                ...
                // The ISOLatin1 table only has 2 byte tokens
                const uint8_t* pSrc = ISOLatin1::ToUTF8Table[uTemp - 0x80U];
                if (pWorkPtr < pEndPtr) {
                    // Write the first character
                    pWorkPtr[0] = pSrc[0];
                }
        ...
        do {
            ...
            if (uTemp < 0x80U) {
                ...
                if (uTemp < 0x10000U) {
                    ...
                    if (pWorkPtr < pEndPtr) {
                        pWorkPtr[0] =
                            static_cast<uint8_t>((uTemp >> 6U) | 0x80U);
                    }
                    ++pWorkPtr;
...
/*! ************************************
    ...
    \return The unsigned 8 bit character code (0x00-0xFF) or Burger::UTF8::BAD
        if the UTF8 value wasn't low ASCII and couldn't be found in the table.
