...
/*! ************************************
	...
	The character map for Win1252 <a href="windows1252.htm">looks like this.
	Click here.</a>
...
/*! ************************************
	...
	\param pInput Pointer to UTF8 buffer that contains the 1 to 3 byte buffer to
		convert. \ref NULL will page fault.
...
/*! ************************************
	...
	\param pOutput Pointer to byte buffer to receive the Win1252 encoded string.
		\ref NULL is okay if uOutputSize is zero, otherwise it will page fault.
...
Word BURGER_API Burger::Win1252::FromUTF8(
	char* pOutput, WordPtr uOutputSize, const char* pInput)
{
	Word8* pWorkPtr = reinterpret_cast<Word8*>(pOutput);

	WordPtr AddZero =
		uOutputSize;   // If nonzero, then I append a 0 to the string.
	if (uOutputSize) { // Valid?
		--uOutputSize;
	}
	Word8* pEndPtr =
		pWorkPtr + uOutputSize; // This is the end of the valid buffer
			...
			if (First < 0x80U) {		  // ASCII?
				...
				do {
					if ((First == pTranslateTable2[0][0]) &&
						...
						if ((reinterpret_cast<const Word8*>(pInput)[1] ==
								Temp)) {
...
/*! ************************************
	...
	\param pOutput Pointer to a byte buffer to receive the Win1252 string. \ref
		NULL is okay if uOutputSize is zero, otherwise a page fault will occur.
	\param uOutputSize Size of the output buffer in bytes.
	\param pInput UTF8 encoded byte array. \ref NULL is okay if uInputSize is
		zero.
	...
	if (uInputSize) { // Sanity check
		do {
			...
			if (First < 0x80U) {		  // ASCII?
				...
				(uInputSize >= 2)) { // Must have at least 2 bytes pending
