...
/*! ************************************
	...
	Windows XP, NT, etc. This class will allow the conversion of
	other character mappings to Win1252 for use in calling
	Win32 API functions.
...
/*! ************************************
	...
	\var const Word8 Burger::Win1252::UpperCaseTable[256]
	\brief Table to convert all characters to upper case.
	
	Using Win1252 mapping, this table will convert all 256 codes into
	their upper case equivalents.
	
***************************************/

const Word8 BURGER_ALIGN(Burger::Win1252::UpperCaseTable[256],16) = {
	...
	0xD0,0xD1,0xD2,0xD3,0xD4,0xD5,0xD6,0xF7,0xD8,0xD9,0xDA,0xDB,0xDC,0xDD,0xFE,0xFF 	// 0xF0
};

/*! ************************************

	\var const Word8 Burger::Win1252::LowerCaseTable[256]
...
const Word8 BURGER_ALIGN(Burger::Win1252::LowerCaseTable[256],16) = {
	0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F,	// 0x00
	0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1A,0x1B,0x1C,0x1D,0x1E,0x1F,	// 0x10
	0x20,0x21,0x22,0x23,0x24,0x25,0x26,0x27,0x28,0x29,0x2A,0x2B,0x2C,0x2D,0x2E,0x2F,	// 0x20
	0x30,0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,0x3A,0x3B,0x3C,0x3D,0x3E,0x3F,	// 0x30
	...
	0xF0,0xF1,0xF2,0xF3,0xF4,0xF5,0xF6,0xD7,0xF8,0xF9,0xFA,0xFB,0xFC,0xFD,0xDE,0xDF,	// 0xD0
...
/*! ************************************
	...
	\var const Word8 Burger::Win1252::ToUTF8Table[128][4]
	\brief Table to convert Win1252 to UTF8.

	This 128x4 array holds the 128 high ascii codes for Win1252 converted
	to UTF8. Some unprintable codes don't exist and are
...
const Word8 BURGER_ALIGN(Burger::Win1252::ToUTF8Table[128][4],16) = {
...
/*! ************************************
	...
	Win1252 character. Codes 0 through 0x7f are considered ASCII while codes
	0x80 through 0xFF are mapped to Win1252. If the UTF8 character cannot be mapped to
	Win1252 encoding, Burger::Win1252::BAD will be returned instead.
	...
	\return The unsigned 8 bit character code (0x00-0xFF) or Burger::Win1252::BAD if the
		UTF8 value wasn't low ASCII and couldn't be mapped to Win1252.
	\sa Burger::UTF8::NextToken(const char *) or Burger::UTF8::GetTokenSize(const char *)
	
***************************************/

Word BURGER_API Burger::Win1252::FromUTF8(const char *pInput)
{
	...
	return static_cast<Word>(Burger::Win1252::BAD);		// Crud
}
...
/*! ************************************
	...
	into a Win1252 encoded "C" string. The function will return the size of the string
	...
	\param pOutput Pointer to byte buffer to receive the Win1252 encoded string. \ref NULL is okay if uOutputSize is zero, otherwise it will page fault.
...
Word BURGER_API Burger::Win1252::FromUTF8(char *pOutput,WordPtr uOutputSize,const char *pInput)
{
...
/*! ************************************
	...
	to a Win1252 encoded "C" string. The function will return the size of the string
	after encoding. This size is valid, even if it exceeded the output buffer size.
	The output pointer and size can be \ref NULL to have this routine calculate the size
	of the possible output so the application can allocate a buffer large enough
	to hold it.
	...
	was parsed. Zeros will be placed in the Win1252 stream as is.
	
	\param pOutput Pointer to a byte buffer to receive the Win1252 string. \ref NULL is okay
...
Word BURGER_API Burger::Win1252::FromUTF8(char *pOutput,WordPtr uOutputSize,const char *pInput,WordPtr uInputSize)
{
	Word8 *pWorkPtr = reinterpret_cast<Word8 *>(pOutput);	
