...
void BURGER_API Burger::LoadHighPrecisionFromFloat(const FPInfo* pFPInfo,
    FPLargeInt* pIntegerPart, FPLargeInt* pFractionalPart) BURGER_NOEXCEPT
{
    ...
    if (!pFPInfo->IsZero()) {
        ...
        pIntegerPart->InsertBitsAtEnd(uLow32Bits,
            (32 + 1) - pFPInfo->GetMantissaBitCount() + pFPInfo->GetExponent());
        pFractionalPart->InsertBitAtStart(uLow32Bits,
            (pFPInfo->GetMantissaBitCount() - 1) - pFPInfo->GetExponent());
        ...
        if (pFPInfo->GetMantissaBitCount() > 32) {
            ...
            pIntegerPart->InsertBitsAtEnd(uHigh32Bits,
                (64 + 1) - pFPInfo->GetMantissaBitCount() +
                    pFPInfo->GetExponent());
...
const char* Burger::FPPrintInfo::g_SpecialFloatsText[] = {
    "inf", "-inf", "nan", "-nan", "-nan"};
...
/*! ************************************
    ...
    For compatibility with some compiler run time libraries, we can stop writing
    the digits of a floating point number after a specified number of digits.
    Set this to 0 to print ALL digits allowed by the conversion specification.

***************************************/

uint_t Burger::FPPrintInfo::g_uClipFloatDigitsAfter = kConversionDigitUnlimited;
...
void BURGER_API Burger::FPPrintInfo::Reset(void) BURGER_NOEXCEPT
{
...
/*! ************************************
    ...
    \fn uint_t Burger::FPPrintInfo::GetIntegerZeroCount(void) const
...
/*! ************************************
    ...
    \fn uint_t Burger::FPPrintInfo::GetFracZeroCount(void) const
...
/*! ************************************
    ...
    \fn uint_t Burger::FPPrintInfo::GetClip(void)
...
/*! ************************************
    ...
    \fn void Burger::FPPrintInfo::SetClip(uint_t uNewClip)
    ...
    const FPInfo* pFPInfo, uint_t uPrecisionDigits) BURGER_NOEXCEPT
{
    ...
    if (pFPInfo->IsZero()) {
        ...
        m_ePrintResult = pFPInfo->IsNegative() ? kResultNegativeInfinity :
                                                 kResultPositiveInfinity;
        m_uSpecialFormDigits = static_cast<uint_t>(StringLength(
            g_SpecialFloatsText[pFPInfo->IsNegative() ? kNegInfinity :
                                                        kPosInfinity]));

        // Special Case: NAN
    } else if (pFPInfo->IsNAN()) {
        m_ePrintResult =
            (pFPInfo->IsNegative()) ? kResultNegativeNaN : kResultPositiveNaN;
        m_uSpecialFormDigits = static_cast<uint_t>(StringLength(
            g_SpecialFloatsText[pFPInfo->IsNegative() ? kNegNan : kPosNan]));
    } else {

        // How many bits do we need to express this number?
        const uint_t uBitsNeeded =
            (1U << (pFPInfo->GetExponentBitCount() - 1)) +
        ...
        m_ePrintResult = kResultNormalFloat;
        m_bHasIntegerPart = IntegerPart.IsNotZero();
        // m_uFirstNonZeroIntDigitPos = 0;

        // Now process all the integer digits, from least significant to most
        // (right to left) loop at least once, to get a leading zero.  Get a
        // count of the number of digits, and the position of the first non-zero
        // digit (from the decimal point)

        uint_t bNonZeroDigitFound = FALSE;
        uint_t bIntPartisNotAll9s = FALSE;
        uint_t uIntDigitsReturned = 0;

        const uint_t bRTLCutOffEmulation = (g_uClipFloatDigitsAfter != 0);
        uint_t uNineCount = 0;
        int_t iLastNonNineDigit = -1;
        ...
        while (IntegerPart.IsNotZero()) {
            ...
            // We track all 9's for the special case when we round all 9s up and
            // create an extra digit
            if (!bIntPartisNotAll9s && (uTheDigit != 9)) {
                bIntPartisNotAll9s = TRUE;
            }

            // for RTL cutoff emulation, we track the last run of 9s and the
            // digit prior to it.
            if (uTheDigit != 9) {
                uNineCount = 0;
                iLastNonNineDigit = static_cast<int_t>(uTheDigit);
            } else {
                ++uNineCount;
            }
        }
        ...
        if (bRTLCutOffEmulation &&
            (uIntDigitsReturned >= g_uClipFloatDigitsAfter)) {

            // we're going to just put zeros in any fractional portion
            m_uPadFracZeros = uPrecisionDigits;

            // do we need to look at the fractional portion to get the rounding
            // digit?
            int_t iRoundingDigit =
                (uIntDigitsReturned == g_uClipFloatDigitsAfter) ?
                static_cast<int_t>(FractionalPart.MultiplyReturnOverflow(10)) :
                -1;

            // Wacky Edge case:  If we are emulating a RTL that cuts off digits
            // (by just emitting 0s) after a certain number of digits ( MSVC
            // does this at 17 significant digits, not including any leading
            // zero),   we could conceivably have all 9s up to the cut off
            // point, in which case we would want to check the first zeroed
            // digit, and possibly round up, adding a whole digit to the output
            // width. We have to do this without any dynamic allocations. *sigh*

            if (uNineCount >= g_uClipFloatDigitsAfter) {
                // should we round it up?
                if ((uNineCount > g_uClipFloatDigitsAfter) ||
                    (iLastNonNineDigit >= 5) || (iRoundingDigit >= 5)) {
                    // add a leading digit to the int part, it's '1' plus some
                    // number of zeros now
                    ++m_uLastNonZeroIntDigitPos;
                    m_uFirstNonZeroIntDigitPos = m_uLastNonZeroIntDigitPos;
                    m_ePrintResult = kResultFloatRoundedUpADigit;
            ...
            // it wont expand via rounding, but we do need to know if we need to
            // round up some number of digits at the end.   And we didn't save
            // all the digits, so...

            // do we need to re-extract the digits?
            if (uIntDigitsReturned > g_uClipFloatDigitsAfter) {
                // Reload the integer portion
                LoadHighPrecisionFromFloat(
                    pFPInfo, &IntegerPart, &FractionalPart);
                uint_t uDigitNeeded =
                    uIntDigitsReturned - g_uClipFloatDigitsAfter;

                // loop until we get the desired digit
                // note: we could optimize this for high digit positions bu
                // diving against 	  larger power of 10
                while (uDigitNeeded) {
                    iRoundingDigit = static_cast<int_t>(
                        IntegerPart.DivideReturnRemainder(10));
                    --uDigitNeeded;
                }
            }

            // mark up the results for the valid digit range, and add the fact
            // that we have padding digits
            m_uFirstNonZeroIntDigitPos =
                uIntDigitsReturned - g_uClipFloatDigitsAfter + 1;
            m_uPadIntZeros = m_uFirstNonZeroIntDigitPos - 1;

            // will we need to round up the result?
            if (iRoundingDigit >= 5) {
                m_ePrintResult = kResultFloatRoundedUpAtEnd;
            }
            return;
        }

        // Now process all the fractional digits, from most significant to least
        // (left to right) Get a count of the number of digits, and the position
        // of the first non-zero digit (right from the decimal point) and the
        // last non-zero digit
        ...
        if (bRTLCutOffEmulation) {
            const uint_t uDigitsRemaing =
                g_uClipFloatDigitsAfter - uIntDigitsReturned;
            if (uPrecisionDigits > uDigitsRemaing) {
                // fake a lesser precision to get correct rounding up and note
                // how many zeros to tack on the end
        ...
        if (FractionalPart.IsNotZero()) {
            ...
            if (uTheDigit >= 5) {
                ...
                if (!bFracPartIsNotAll9s && !bIntPartisNotAll9s) {
                    // add a digit to the int part, it's '1' plus some number of
                    // zeros now
                    ++m_uLastNonZeroIntDigitPos;
                    m_uFirstNonZeroIntDigitPos = m_uLastNonZeroIntDigitPos;
                    m_ePrintResult = kResultFloatRoundedUpADigit;
...
/*! ************************************
    ...
    \code
    SafePrintf("Float
%hf\n",SafePrintArgument(fHalf,SafePrintArgument::ARG_HALF)); \endcode
...
/*! ************************************
    ...
    \param arg Pointer to data that wasn't already captured by a specific
constructor
...
/*! ************************************
    ...
    \fn uint_t Burger::SafePrintArgument::IsNumeric(void) const
...
/*! ************************************
    ...
    \fn uint_t Burger::SafePrintArgument::IsInteger(void) const
...
/*! ************************************
    ...
    \fn uint_t Burger::SafePrintArgument::IsSigned(void) const
...
/*! ************************************
    ...
    \fn uint_t Burger::SafePrintArgument::IsUnsigned(void) const
...
/*! ************************************
    ...
    \fn uint_t Burger::SafePrintArgument::IsBoolean(void) const
...
/*! ************************************
    ...
    \fn uint_t Burger::SafePrintArgument::IsReal(void) const
...
uint_t BURGER_API Burger::SafePrintArgument::IsNegative(
...
uint_t BURGER_API Burger::SafePrintArgument::IsZero(void) const BURGER_NOEXCEPT
{
    uint_t bResult;
    switch (m_eType) {
    case ARG_INT8:
...
/*! ************************************
    ...
    \fn uint_t Burger::SafePrintArgument::IsCharacter(void) const
...
/*! ************************************
    ...
    \fn uint_t Burger::SafePrintArgument::IsTextPointer(void) const
...
/*! ************************************
    ...
    \fn uint_t Burger::SafePrintArgument::IsPointer(void) const
...
/*! ************************************
    ...
    \fn uint_t Burger::SafePrintArgument::Is64Bit(void) const
...
/*! ************************************
    ...
    \fn uint_t Burger::SafePrintArgument::IsSIMD(void) const
...
/*! ************************************
    ...
    \fn uint_t Burger::SafePrintArgument::IsSIMDPointer(void) const
...
uintptr_t BURGER_API Burger::SafePrintArgument::GetDataLengthInBytes(
    void) const BURGER_NOEXCEPT
{
    uintptr_t uResult;
    switch (m_eType) {
...
const void* BURGER_API Burger::SafePrintArgument::GetDataAddress(
    void) const BURGER_NOEXCEPT
{
...
/*! ************************************
    ...
    Example: int8_t data, with an enumeration of \ref ARG_INT8 returns "int8_t"
...
const char* BURGER_API Burger::SafePrintArgument::GetTypeName(
    eType uType) BURGER_NOEXCEPT
{
    const char* pResult;
    switch (uType) {
    case ARG_INT8:
...
/*! ************************************
    ...
    \fn const char* Burger::SafePrintArgument::GetTypeName(void) const
...
int32_t BURGER_API Burger::SafePrintArgument::GetInt32(void) const BURGER_NOEXCEPT
{
    int32_t iResult;
    switch (m_eType) {
    case ARG_INT8:
    ...
    case ARG_UINT8:
        iResult = static_cast<int32_t>(m_Data.m_uWord8);
        break;
    case ARG_UINT16:
        iResult = static_cast<int32_t>(m_Data.m_uWord16);
        break;
    case ARG_UINT32:
        iResult = static_cast<int32_t>(m_Data.m_uWord32);
        break;
    case ARG_UINT64:
        iResult = static_cast<int32_t>(m_Data.m_uWord64);
        break;
...
int64_t BURGER_API Burger::SafePrintArgument::GetInt64(
    void) const BURGER_NOEXCEPT
{
    int64_t iResult;
    switch (m_eType) {
    ...
    case ARG_UINT8:
        iResult = static_cast<int64_t>(m_Data.m_uWord8);
        break;
    case ARG_UINT16:
        iResult = static_cast<int64_t>(m_Data.m_uWord16);
        break;
    case ARG_UINT32:
        iResult = static_cast<int64_t>(m_Data.m_uWord32);
        break;
    case ARG_UINT64:
        iResult = static_cast<int64_t>(m_Data.m_uWord64);
        break;
...
uint32_t BURGER_API Burger::SafePrintArgument::GetUInt32(
    void) const BURGER_NOEXCEPT
{
    uint32_t uResult;
    switch (m_eType) {
    ...
    case ARG_UINT32:
        ...
        break;
    case ARG_UINT64:
        uResult = static_cast<uint32_t>(m_Data.m_uWord64);
        break;
...
uint64_t BURGER_API Burger::SafePrintArgument::GetUInt64(
    void) const BURGER_NOEXCEPT
{
    uint64_t uResult;
    switch (m_eType) {
...
char BURGER_API Burger::SafePrintArgument::GetChar(void) const BURGER_NOEXCEPT
{
    char iResult;
    switch (m_eType) {
    case ARG_INT8:
...
uint_t BURGER_API Burger::SafePrintArgument::GetBool(void) const BURGER_NOEXCEPT
{
...
const void* BURGER_API Burger::SafePrintArgument::GetPointer(
    void) const BURGER_NOEXCEPT
{
...
const char* BURGER_API Burger::SafePrintArgument::GetText(
    void) const BURGER_NOEXCEPT
{
    const char* pResult;
...
uintptr_t BURGER_API Burger::SafePrintArgument::GetTextLength(
    void) const BURGER_NOEXCEPT
{
    uintptr_t uResult;
    if (m_eType == ARG_PSTRING) {
...
/*! ************************************
    ...
    \fn void Burger::SafePrint::ParamInfo_t::SetFlags(uint32_t uFormatOptionFlags)
...
enforces type safety
    ...
    \sa GetFloatInfo(uint_t *,uint_t *,uint_t *,uint_t *,uint_t *,uint_t *) const
...
/*! ************************************
    ...
    \sa GetFloatAnalysisInfo(FPPrintInfo::eResult *,uint_t *,uint_t *,uint_t *) const
    ...
    if (uConversionSpecifier == CONVSPEC_BINNUMBER) {
        ...
        if (TheFPPrintInfo.IsSpecial()) {
            ...
            if (uConversionSpecifier == CONVSPEC_DECIMAL_FLOAT) {
                ...
                SetFloatAnalysisInfo(TheFPPrintInfo.GetResult(),
                    TheFPPrintInfo.GetIntegerZeroCount(),
                    TheFPPrintInfo.GetFracZeroCount(),
...
Burger::SafePrint::eError BURGER_API
Burger::SafePrint::ParamInfo_t::StoreOffNumericValue(
...
uintptr_t BURGER_API Burger::SafePrint::ParamInfo_t::FormatInteger(
    char* pOutBuffer, const SafePrintArgument* pArg) const BURGER_NOEXCEPT
{
    // sanity checks
    ...
    char* pOutBuffer, const SafePrintArgument* pArg) const BURGER_NOEXCEPT
{
    ...
    SafePrintArgument TempArg(reinterpret_cast<uint32_t>(pArg->m_Data.m_pVoid));
    TempArg.SetType(SafePrintArgument::ARG_UINT32);
#else
    SafePrintArgument TempArg(reinterpret_cast<uint64_t>(pArg->m_Data.m_pVoid));
    TempArg.SetType(SafePrintArgument::ARG_UINT64);
#endif
...
uintptr_t BURGER_API Burger::SafePrint::ParamInfo_t::FormatChar(
    char* pOutBuffer,
    const Burger::SafePrintArgument* pArg) const BURGER_NOEXCEPT
{
    // sanity checks
    ...
    char* pOutBuffer, const SafePrintArgument* pArg) const BURGER_NOEXCEPT
{
    ...
    if (uCharsWritten) {
        if (pArg->GetType() == SafePrintArgument::ARG_PUINT16) {
            if (pArg->m_Data.m_pWord16) {
                const char Temp = pOutText[uCharsWritten];
                UTF8::FromUTF16(
                    pOutText, uCharsWritten + 1, pArg->m_Data.m_pWord16);
    ...
    char* pOutBuffer, FPPrintInfo::eResult uSpecialResult) const BURGER_NOEXCEPT
{
    ...
    const char* pText = (IsFlagSet(CONVFLAG_UPPERCASE)) ?
        ...
        FPPrintInfo::g_SpecialFloatsText[uIndex];
    ...
    const SafePrintArgument* pArgument) BURGER_NOEXCEPT
{
    ...
    if (IsPointerInvalid(pOutputBuffer) || IsPointerInvalid(pParamInfo) ||
        ...
        const SafePrintArgument::eType uArgumentType = pArgument->GetType();
...
uint_t BURGER_API Burger::SafePrint::ProcessResults_t::WriteBufferToFile(
    FILE* fp, uint_t bUsingSTDOUT, const char* pBuffer, uintptr_t uBufferSize,
    uintptr_t uCharsSoFar) BURGER_NOEXCEPT
{
    ...
    if (uBufferSize) {
        ...
        if (uCharsPassed != uBufferSize) {
            m_uPhaseResults =
                (bUsingSTDOUT) ? ERROR_WRITING_STDOUT : ERROR_WRITING_TO_FILE;
...
/*! ************************************
    ...
    \param bUsingSTDOUT \ref TRUE if stdin or stdout, \ref FALSE if a different
        stream or file
    \param uArgCount Number of available arguments
    \param ppArgs Pointer to the array of pointers to arguments
    \param uParamInfoCount Number of parameter commands to process
...
Burger::SafePrint::ProcessResults_t::GenerateFormattedOutputToFile(FILE* fp,
    uint_t bUsingSTDOUT, uintptr_t uArgCount, const SafePrintArgument** ppArgs,
    uintptr_t uParamInfoCount, const ParamInfo_t* pParamInfos) BURGER_NOEXCEPT
{
    ...
    for (uintptr_t x = 0; x < uParamInfoCount; x++) {
        ...
        case COMMAND_COPY_TEXT: {
            ...
            if (pParamInfos->IsFlagSet(CONVFLAG_HAS_SEQUENCE_CHAR)) {
                ...
                while (uSrcBytesRemaining) {
                    ...
                    if (uBufferUsed) {
                        if (!WriteBufferToFile(fp, bUsingSTDOUT, pSrcText,
                                uBufferUsed, uCharCount)) {
                            return uCharCount;
                        }
                ...
                if (!WriteBufferToFile(
                        fp, bUsingSTDOUT, pSrcText, uBytesToCopy, uCharCount)) {
                    return uCharCount;
                }
                uCharCount += uBytesToCopy;
            }
        ...
        case COMMAND_FORMAT_ARGUMENT: {
            ...
            if (!WriteBufferToFile(
                    fp, bUsingSTDOUT, Buffer, uCharsAdded, uCharCount)) {
                return uCharCount;
            }
            uCharCount += uCharsAdded;
            break;
    ...
    if ((uOptions & PARSEOPTION_CHECK_ARG_TYPES) && uArgCount) {
        ...
        for (uintptr_t i = 0; i < uArgCount; i++) {
            if (IsPointerInvalid(ppArgs[i]) ||
                (ppArgs[i]->GetType() == SafePrintArgument::ARG_INVALID) ||
                (ppArgs[i]->GetType() >= SafePrintArgument::ARG_COUNT)) {
            ...
            if (uOpCode == OP_NUMERIC_VALUE) {
                ...
                if ((uOpCode >= OP_OUTPUT_BEGIN) &&
                    ...
                    if (uOptions & PARSEOPTION_CHECK_ARG_TYPES) {
                        ...
                        // (ArgType::xxx)
                        const SafePrintArgument::eType uParameterType =
...
const uint32_t Burger::SafePrint::g_ValidOutputDisplayTypesStrict
    [SafePrintArgument::ARG_COUNT] = {
        ...
        CONVSPECFLAG_BINARY_NUMBER | CONVSPECFLAG_DECIMAL_FLOAT |
            ...
            CONVSPECFLAG_HEX_FLOAT, // ARG_HALF
        CONVSPECFLAG_BINARY_NUMBER | CONVSPECFLAG_DECIMAL_FLOAT |
            CONVSPECFLAG_SCINOTE_FLOAT | CONVSPECFLAG_SHORTER_FLOAT |
            CONVSPECFLAG_HEX_FLOAT, // ARG_FLOAT
        CONVSPECFLAG_BINARY_NUMBER | CONVSPECFLAG_DECIMAL_FLOAT |
            CONVSPECFLAG_SCINOTE_FLOAT | CONVSPECFLAG_SHORTER_FLOAT |
            CONVSPECFLAG_HEX_FLOAT, // ARG_DOUBLE
...
const char* BURGER_API Burger::SafePrint::GetErrorDescription(
    eError uType) BURGER_NOEXCEPT
{
    ...
    case ERROR_WRITING_TO_FILE:
        pResult =
            "Error occurred in RTL function fprintf while writing formatted output to file";
        break;
    case ERROR_WRITING_STDOUT:
        pResult =
            "Error occurred in RTL function printf while writing formatted output to stdout";
        break;
    case ERROR_RTL_TRANSMITTED_COUNT:
        pResult =
            "The RTL function used to transmit formatted output reported a different amount of chars sent";
        break;
    ...
    const SafePrintArgument* pArg) BURGER_NOEXCEPT
{
    ...
    if (pArg) {
        ...
        case SafePrintArgument::ARG_UINT8:
            uCode = OP_OUTPUT_DECIMALWORD;
            break;
        case SafePrintArgument::ARG_UINT16:
            uCode = OP_OUTPUT_DECIMALWORD;
            break;
        case SafePrintArgument::ARG_UINT32:
            uCode = OP_OUTPUT_DECIMALWORD;
            break;
        case SafePrintArgument::ARG_UINT64:
            uCode = OP_OUTPUT_DECIMALWORD;
            break;
        ...
        case SafePrintArgument::ARG_PINT16:
            ...
            break;
        case SafePrintArgument::ARG_PUINT16:
            uCode = OP_OUTPUT_POINTER;
            break;
        case SafePrintArgument::ARG_PINT32:
            uCode = OP_OUTPUT_POINTER;
            break;
        case SafePrintArgument::ARG_PUINT32:
            uCode = OP_OUTPUT_POINTER;
            break;
        case SafePrintArgument::ARG_PINT64:
            uCode = OP_OUTPUT_POINTER;
            break;
        case SafePrintArgument::ARG_PUINT64:
            uCode = OP_OUTPUT_POINTER;
            break;
    ...
    uintptr_t uArgCount, const SafePrintArgument** ppArgs) BURGER_NOEXCEPT
{
    ...
    if (bParseOk &&
        (TheProcessResults.m_uPhaseResults == SafePrint::ERROR_NONE)) {
...
/*! ************************************
    ...
    * * Step 3) If no issues found in Step 1 and a buffer was returned in step
    ...
    \sa Sprintf(char *,const char *,uintptr_t,const SafePrintArgument **)
    ...
    const SafePrintArgument** ppArgs) BURGER_NOEXCEPT
{
    ...
    const uint_t bNoErrors = (bParseOk &&
        TheProcessResults.m_uPhaseResults == SafePrint::ERROR_NONE);
...
/*! ************************************
    ...
    * * Step 2) if no issues found in Step 1, execute the list of format & copy
    ...
    \sa Snprintf(char *,uintptr_t,const char *,uintptr_t,const SafePrintArgument **)
        or SprintfUserAlloc(SafePrint::SprintfCallbackProc,void *,uint_t,const
        char *,uintptr_t,const SafePrintArgument **)
    ...
    const SafePrintArgument** ppArgs) BURGER_NOEXCEPT
{
    ...
    if (bParseOk &&
        ...
        const uintptr_t uProcessedCount =
            TheProcessResults.GenerateFormattedOutputToFile(stdin, TRUE,
                uArgCount, ppArgs, TheProcessResults.m_uNumOutputCommands,
                ParamIntoTable);
    ...
    uintptr_t uArgCount, const SafePrintArgument** ppArgs) BURGER_NOEXCEPT
{
    ...
    if (bParseOk &&
        ...
        // format the output and send it along
        const uintptr_t uProcessedCount =
            TheProcessResults.GenerateFormattedOutputToFile(fp, FALSE,
                uArgCount, ppArgs, TheProcessResults.m_uNumOutputCommands,
                ParamIntoTable);
    ...
    const SafePrintArgument** ppArgs) BURGER_NOEXCEPT
{
    ...
    printf("Results:  Error='%s' at char pos #%zd (char %zd in sequence)\n",
        SafePrint::GetErrorDescription(TheProcessResults.m_uPhaseResults),
        TheProcessResults.m_uErrorCharPos,
