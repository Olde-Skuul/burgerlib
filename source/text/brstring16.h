...
public:
	...
	static const Word BUFFERSIZE = static_cast<Word>(
		(128 - (sizeof(Word16*) + sizeof(WordPtr))) / sizeof(Word16));
...
public:
	String16(void) : m_pData(m_Raw), m_uLength(0)
	{
		m_Raw[0] = 0;
	}
	String16(const String16& rInput);
	String16(const String16& rInput, WordPtr uStart,
		WordPtr uEnd = UINTPTR_MAX);
	...
	BURGER_INLINE const Word16* c_str() const
	{
		return m_pData;
	}
	BURGER_INLINE Word16* c_str()
	{
		return m_pData;
	}
	...
	BURGER_INLINE Word16* GetPtr()
	{
		return m_pData;
	}
	BURGER_INLINE const Word16* GetPtr() const
	{
		return m_pData;
	}
	...
	BURGER_INLINE Word16& operator()(WordPtr uInput)
	{
		if (uInput >= m_uLength) {
