...
/*! ************************************
	...
	\var const uint8_t Burger::g_reverse_bits[256]
...
BURGER_ALIGN(const uint8_t, Burger::g_reverse_bits[256], 16) = {
...
/*! ************************************
	...
	If the character is ASCII SPACE, TAB, LF, VT, FF or CR, return non zero,
...
uint64_t BURGER_API Burger::BitReverse(
...
uint_t BURGER_API Burger::BitSetCount(uint32_t uInput) BURGER_NOEXCEPT
{
...
uint_t BURGER_API Burger::BitSetCount(uint64_t uInput) BURGER_NOEXCEPT
{
...
void BURGER_API Burger::CStringToPString(
...
void BURGER_API Burger::PStringToCString(
...
char* BURGER_API Burger::ParseBeyondWhiteSpace(
...
char* BURGER_API Burger::ParseBeyondEOL(
...
uintptr_t BURGER_API Burger::CopyUpToEOL(char* pOutput, uintptr_t uOutputSize,
	const char* pInput, uintptr_t uInputSize) BURGER_NOEXCEPT
{
	...
	if (uInputSize) { // Any input data?
		...
		// Make a copy of the output buffer size
		uintptr_t uOutSize = uOutputSize;
...
void BURGER_API Burger::StripLeadingSpaces(char* pInput) BURGER_NOEXCEPT
{
...
void BURGER_API Burger::StripLeadingAndTrailingSpaces(
...
void BURGER_API Burger::StripLeadingAndTrailingWhiteSpace(
	char* pInput) BURGER_NOEXCEPT
{
	char* pEnd = pInput; // Save the starting point of the string
	uint_t uTemp = reinterpret_cast<uint8_t*>(pInput)[0];
...
void BURGER_API Burger::StripAllFromList(
...
void BURGER_API Burger::StripAllButList(
	char* pInput, const char* pList) BURGER_NOEXCEPT
{
	if (reinterpret_cast<uint8_t*>(pInput)[0]) { // Any input
		...
		if (reinterpret_cast<const uint8_t*>(pList)[0]) {
			...
			do {
				...
				const uint8_t* pTempList =
					reinterpret_cast<const uint8_t*>(pList);
				do {
					++pTempList;
					if (uTemp2 == uTemp) {
						pTemp[0] = static_cast<char>(uTemp); // Accept the char
...
void BURGER_API Burger::StripTrailing(
	char* pInput, const char* pList) BURGER_NOEXCEPT
{
...
void BURGER_API Burger::StripLeading(
	char* pInput, const char* pList) BURGER_NOEXCEPT
{
	uint_t uTemp = reinterpret_cast<const uint8_t*>(pInput)[0];
	if (uTemp && reinterpret_cast<const uint8_t*>(pList)[0]) { // Any input
...
void BURGER_API Burger::SlashesToColons(char* pInput) BURGER_NOEXCEPT
{
...
void BURGER_API Burger::SlashesToWindowsSlashes(char* pInput) BURGER_NOEXCEPT
{
	uint_t uTemp = reinterpret_cast<uint8_t*>(pInput)[0];
	if (uTemp) {
		do {
...
void BURGER_API Burger::EndWithWindowsSlashes(char* pInput) BURGER_NOEXCEPT
{
...
void BURGER_API Burger::EndWithWindowsSlashes(
	char* pOutput, const char* pInput) BURGER_NOEXCEPT
{
	uint_t uTemp = reinterpret_cast<const uint8_t*>(pInput)[0];
	uint_t uLastChar = 0;
...
void BURGER_API Burger::SlashesToLinuxSlashes(char* pInput) BURGER_NOEXCEPT
{
	uint_t uTemp = reinterpret_cast<uint8_t*>(pInput)[0];
	if (uTemp) {
		do {
...
void BURGER_API Burger::SlashesToLinuxSlashes(
	char* pOutput, const char* pInput) BURGER_NOEXCEPT
{
	uint_t uTemp = reinterpret_cast<const uint8_t*>(pInput)[0];
	if (uTemp) {
...
void BURGER_API Burger::EndWithLinuxSlashes(char* pInput) BURGER_NOEXCEPT
{
	// Get the initial character
	uint_t uTemp = reinterpret_cast<uint8_t*>(pInput)[0];
	uint_t uLastChar = 0;
...
void BURGER_API Burger::EndWithLinuxSlashes(
	char* pOutput, const char* pInput) BURGER_NOEXCEPT
{
	uint_t uTemp = reinterpret_cast<const uint8_t*>(pInput)[0];
	uint_t uLastChar = 0;
...
void BURGER_API Burger::Replace(
	char* pOutput, const char* pInput, uint_t uFrom, uint_t uTo) BURGER_NOEXCEPT
{
	uint_t uTemp = reinterpret_cast<const uint8_t*>(pInput)[0];
	if (uTemp) {
		do {
	...
	const char* pInput1, const char* pInput2) BURGER_NOEXCEPT
{
	...
	do {
		...
		++pInput1;
		// Second byte
		const uint_t uTemp2 = reinterpret_cast<const uint8_t*>(pInput2)[0];
		++pInput2;
	...
	const uint16_t* pInput1, const uint16_t* pInput2) BURGER_NOEXCEPT
{
	...
	do {
		...
		++pInput1;
		// Second byte
		const uint_t uTemp2 = pInput2[0];
		++pInput2;

		// Compare
		iTemp = static_cast<int>(uTemp1 - uTemp2);
		if (iTemp) {
			break;
		}
	} while (uTemp1); // End of string? (And match!!)
...
void BURGER_API Burger::StringUppercase(
	char* pOutput, const char* pInput) BURGER_NOEXCEPT
{
	// Prefetch first char
	uint_t uTemp = reinterpret_cast<const uint8_t*>(pInput)[0];
...
char* BURGER_API Burger::StringString(
...
char* BURGER_API Burger::StringCaseString(
	const char* pInput, const char* pTest) BURGER_NOEXCEPT
{
	// Get the first character
	uint_t uTemp = reinterpret_cast<const uint8_t*>(pInput)[0];
