...
#if (__WATCOMC__ == 1280) || \
	(__WATCOMC__ == 1290) // Test for Open Watcom 1.8 or 1.9
...
/*! ************************************
	...
	this flag to suppress the output of the terminating \ref NULL. The default
	behavior is that a zero is output as the last character so that the string
	is a valid "C" string. Pass this value in the uDigits parameter to disable
	the terminating zero output.
...
/*! ************************************
	...
	The functions that convert a float or integer into an ASCII string use this
	flag to control the output of leading zeros. The default behavior is that
	leading zeros are suppressed. Pass this value in the uDigits parameter to
	enable leading zero output.
...
/*! ************************************
	...
	When Burger::g_AsciiTestTable[128] is indexed with an ASCII character, test
	it with this value to determine if it is a lower case character.
...
/*! ************************************
	...
	When Burger::g_AsciiTestTable[128] is indexed with an ASCII character, test
	it with this value to determine if it is a upper case character.
...
/*! ************************************
	...
	When Burger::g_AsciiTestTable[128] is indexed with an ASCII character, test
	it with this value to determine if it is a control character.
...
/*! ************************************
	...
	When Burger::g_AsciiTestTable[128] is indexed with an ASCII character, test
	it with this value to determine if it is an ASCII space character such as
	CR, LF, FF, SPACE and TAB
...
/*! ************************************
	...
	When Burger::g_AsciiTestTable[128] is indexed with an ASCII character, test
	it with this value to determine if it is an ASCII whitespace character which
...
/*! ************************************
	...
	When Burger::g_AsciiTestTable[128] is indexed with an ASCII character, test
	it with this value to determine if it is an ASCII punctuation character such
	as [, ], !, ?, etc.
...
/*! ************************************
	...
	When Burger::g_AsciiTestTable[128] is indexed with an ASCII character, test
	it with this value to determine if it is an ASCII hex character which is
	A-F, a-f, and 0-9.
...
/*! ************************************
	...
	When Burger::g_AsciiTestTable[128] is indexed with an ASCII character, test
	it with this value to determine if it is an ASCII numeric character which is
	0-9.
...
/*! ************************************
	...
	Word32 MatchAIFF(const void *pData)
	{
		return static_cast<const Word32 *>(pData)[0] == AIFFCode;
	}
...
/*! ************************************
	...
	\fn Burger::FourCharacterCode_t::operator = (Word32 uInput)
...
/*! ************************************
	...
	\fn Burger::FourCharacterCode_t::SetCode(Word32 uInput)
	\brief Assign a four character code with an integer.

	\param uInput Four character code in big endian format
	\sa operator = (Word32 uInput)
...
/*! ************************************
	...
	\sa Burger::ASCII_CONTROL,Burger::ASCII_UPPER,Burger::ASCII_LOWER or
		Burger::ASCII_PUNCTUATION
...
/*! ************************************
	...
	This table will take the a Word8 used as an index and return the bit
...
***************************************/
	...
	(defined(BURGER_WATCOM) || (defined(BURGER_FASTCALLENABLED) && !defined(BURGER_CLANG) && !defined(BURGER_GNUC)))
...
#elif ((defined(BURGER_AMD64) || defined(BURGER_ARM)) && \
    defined(BURGER_MSVC)) || \
	(defined(BURGER_INTEL) && \
		(defined(BURGER_GNUC) || defined(BURGER_LLVM)))
...
/*! ************************************
	...
	\fn Word Burger::IsDigit(char iInput)
...
/*! ************************************
	...
	\fn Word Burger::IsHex(char iInput)
...
/*! ************************************
	...
	\fn Word Burger::IsWhitespace(char iInput)
...
/*! ************************************
	...
	\fn Word Burger::IsLowercase(char iInput)
...
/*! ************************************
	...
	\fn Word Burger::IsUppercase(char iInput)
	\brief Test if a character is A-Z
...
#if ((defined(BURGER_WATCOM) || (defined(BURGER_FASTCALLENABLED) && !defined(BURGER_CLANG) && !defined(BURGER_GNUC))) && \
	defined(BURGER_X86)) && \
	!defined(DOXYGEN)

// clang-format off

BURGER_DECLSPECNAKED Word32 BURGER_API Burger::BitReverse(
	Word32 /* uInput */, Word /* uBitLength */)
{
	BURGER_ASM
	{
#if defined(BURGER_WATCOM)
...
Word BURGER_API Burger::BitSetCount(Word64 uInput)
{
	...
	return BitSetCount(static_cast<Word32>(uInput)) +
		BitSetCount(static_cast<Word32>(uInput >> 32U));
#endif
}
...
void BURGER_API Burger::CStringToPString(Word8* pOutput, const char* pInput)
{
	...
	if (uTemp1) { // End of string?
		...
		do {
			...
			++pInput;
			pWork[0] = static_cast<Word8>(uTemp1); // Save to destination string
			++pWork;
			if (++uLength == 255) {
				break; // Overflow!
			}
		...
		pOutput[0] = static_cast<Word8>(uLength);
	}
}
...
char* BURGER_API Burger::ParseBeyondWhiteSpace(const char* pInput)
{
	...
	do {
		uTemp =
			reinterpret_cast<const Word8*>(pInput)[0]; // Get a byte of input
		++pInput;
...
char* BURGER_API Burger::ParseBeyondEOL(const char* pInput, WordPtr uLength)
{
	...
	if (uLength) {
		do {
			uTemp = reinterpret_cast<const Word8*>(
...
{
	Word uLetter =
		reinterpret_cast<const Word8*>(pInput)[0]; // Get the first char
	Word uQuote = ' ';
...
WordPtr BURGER_API Burger::CopyUpToEOL(
	...
	if (uTemp == ' ') {		  // Is there a leading space?
		...
		if (uTemp) { // End of the string?
			do {
				...
				++pTemp;	// the buffer
				uTemp =
					reinterpret_cast<Word8*>(pInput)[0]; // Accept the next char
				++pInput;
...
void BURGER_API Burger::StripAllButList(char* pInput, const char* pList)
{
	if (reinterpret_cast<Word8*>(pInput)[0]) { // Any input
		char* pTemp = pInput;				   // Destination pointer
		if (reinterpret_cast<const Word8*>(
				pList)[0]) { // Is there a keep pointer?
			Word uTemp =
				reinterpret_cast<Word8*>(pInput)[0]; // Grab an input char
			do {
				++pInput;
	...
	if (uTemp && reinterpret_cast<const Word8*>(pList)[0]) { // Any input
		...
		do {
			if (uTest == uTemp) { // Found one?
				...
				pListTemp = reinterpret_cast<const Word8*>(
	...
	char* pInput, const char* pNewExtension)
{
	...
	if (pNewExtension) { // Don't add a new extension?
		Word uTemp = reinterpret_cast<const Word8*>(
...
/*! ************************************
	...
	the constant 0xFEFEFEFF to the \ref Word32.
...
/*! ************************************
	...
	\sa Burger::StringCopy(char *,WordPtr,const char *) or
...
void BURGER_API Burger::StringCopy(char* pOutput, const char* pInput)
{
	...
	// Word32 when I mean to write a Word8
	//
...
/*! ************************************
	...
	\sa Burger::StringCopy(Word16 *,WordPtr,const Word16 *) or
		Burger::StringLength(const Word16 *)
...
/*! ************************************
	...
	\sa Burger::StringCopy(Word16 *,const Word16 *) or
		Burger::StringLength(const Word16 *)
...
/*! ************************************
	...
	\ref Burger::Alloc(WordPtr). The buffer is exactly the same size of the
	...
	\sa Burger::StringDuplicate(const char *,WordPtr) or
		Burger::StringDelete(const char *).
...
/*! ************************************
	...
	The "C" string passed by pInput will be copied into a buffer allocated by
	Burger::Alloc(WordPtr). The buffer is the same size of the string plus the
...
/*! ************************************
	...
	Burger::StringDuplicate(const char *,WordPtr) then you must dispose of it
	with this function.
...
/*! ************************************
	...
	\sa Burger::StringConcatenate(char *,WordPtr,const char *) or
		Burger::StringLength(const char *)
...
/*! ************************************
	...
	\sa Burger::StringConcatenate(Word16 *,WordPtr,const Word16 *) or
		Burger::StringLength(const Word16 *)
...
/*! ************************************
	...
	\sa Burger::StringConcatenate(Word16 *,const Word16 *) or
		Burger::StringLength(const Word16 *)
...
/*! ************************************
	...
	\sa Burger::StringCompare(const char *,const char *),
		Burger::StringCaseCompare(const char *,const char *,WordPtr)
...
{
	Word uTemp =
		reinterpret_cast<const Word8*>(pInput)[0]; // Prefetch first char
	if (uTemp) {								   // End of string?
		do {
			if ((uTemp - 'a') < 26) { // Is it 'a' to 'z' inclusive?
			...
			++pOutput;
			uTemp =
				reinterpret_cast<const Word8*>(pInput)[1]; // Get the next char
			++pInput;
		} while (uTemp); // Still more?
	}
...
/*! ************************************
	...
	\sa Burger::StringCharacter(const Word16 *,Word) and
...
/*! ************************************
	...
	\sa Burger::StringCharacterReverse(const Word16 *,Word),
		Burger::MemoryCharacterReverse(const char *,WordPtr,int) and
...
/*! ************************************
	...
	\sa Burger::StringCharacterReverse(const char *,int),
		Burger::MemoryCharacterReverse(const Word16 *,WordPtr,Word) and
		Burger::StringCharacter(const Word16 *,Word)
...
/*! ************************************
	...
	\sa Burger::StringStopAt(const Word16 *,const Word16 *)
	...
	const char* pInput, const char* pDelimiters) BURGER_NOEXCEPT
{
	...
	if (!pInput || !pDelimiters || !pDelimiters[0]) {
		...
		// Use the offset
		uResult = static_cast<WordPtr>(pWork - pInput);
	}
	// Return the offset
	return uResult;
}
	...
	const Word16* pInput, const Word16* pDelimiters) BURGER_NOEXCEPT
{
	...
	if (!pInput || !pDelimiters || !pDelimiters[0]) {
		...
		// Scan the string
		const Word16* pWork = pInput;
		Word uTemp = pWork[0];
		if (uTemp) {
			do {
				// Scan the character if it's in the delimiter list
				const Word16* pWorkLimit = pDelimiters;
				Word uDelimiter = pWorkLimit[0];
				do {
					// Match a delimiter?
					if (uDelimiter == uTemp) {
...
Word16* BURGER_API Burger::StringString(
...
Word16* BURGER_API Burger::StringCaseString(
	const Word16* pInput, const Word16* pTest)
{
	// Get the first character
	Word uTemp = pInput[0];
...
/*! ************************************
	...
	\sa Burger::StringToken(Word16 *,const Word16 *, Word16 **)
