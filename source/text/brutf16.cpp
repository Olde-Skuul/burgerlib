    ...
    if (uElementCount) {
        do {
            ...
            if ((uFirst >= 0xD800U) && (uFirst < 0xE000U)) {
                ...
                // Get the next value
                const uint_t uSecond = pInput[0];
                --uElementCount;
                ++pInput;
                // The surrogate pair is okay.
                // Out of bounds?
                if ((uSecond < 0xDC00U) || (uSecond >= 0xE000U)) {
                    // This is a bad UTF16 string
                    return FALSE;
                }
            }
...
/*! ************************************
    ...
    \return The UTF16 code or Burger::UTF16::BAD if invalid. 0x00 is not
        invalid.
...
uint16_t BURGER_API Burger::UTF16::FromUTF8(const char* pInput) BURGER_NOEXCEPT
{
...
uintptr_t BURGER_API Burger::UTF16::FromUTF8(uint16_t* pOutput,
