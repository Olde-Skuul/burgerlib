...
/*! ************************************
	...
	MacRomanUS is a high ASCII encoding that's used by the MacOS classic
	and Carbon environments. This class will allow the conversion of
	other character mappings to MacRomanUS for use in calling
	MacOS API functions.
...
/*! ************************************
	...
	\var const Word8 Burger::MacRomanUS::UpperCaseTable[256]
	\brief Table to convert all characters to upper case.
	
	Using MacRomanUS mapping, this table will convert all 256 codes into
	their upper case equivalents.
	
***************************************/

const Word8 BURGER_ALIGN(Burger::MacRomanUS::UpperCaseTable[256],16) = {
	...
	0x80,0x81,0x82,0x83,0x84,0x85,0x86,0xE7,0xCB,0xE5,0x80,0xCC,0x81,0x82,0x83,0xE9,	// 0x80
	0xE6,0xE8,0xEA,0xED,0xEB,0xEC,0x84,0xEE,0xF1,0xEF,0x85,0xCD,0xF2,0xF4,0xF3,0x86,	// 0x90
	0xA0,0xA1,0xA2,0xA3,0xA4,0xA5,0xA6,0xA7,0xA8,0xA9,0xAA,0xAB,0xAC,0xAD,0xAE,0xAF,	// 0xA0
	0xB0,0xB1,0xB2,0xB3,0xB4,0xB5,0xB6,0xB7,0xB8,0xB9,0xBA,0xBB,0xBC,0xBD,0xAE,0xAF,	// 0xB0
	0xC0,0xC1,0xC2,0xC3,0xC4,0xC5,0xC6,0xC7,0xC8,0xC9,0xCA,0xCB,0xCC,0xCD,0xCE,0xCE,	// 0xC0
	0xD0,0xD1,0xD2,0xD3,0xD4,0xD5,0xD6,0xD7,0xD9,0xD9,0xDA,0xDB,0xDC,0xDD,0xDE,0xDF,	// 0xD0
...
/*! ************************************
	...
	\var const Word8 Burger::MacRomanUS::LowerCaseTable[256]
...
const Word8 BURGER_ALIGN(Burger::MacRomanUS::LowerCaseTable[256],16) = {
	0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F,	// 0x00
	0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1A,0x1B,0x1C,0x1D,0x1E,0x1F,	// 0x10
	0x20,0x21,0x22,0x23,0x24,0x25,0x26,0x27,0x28,0x29,0x2A,0x2B,0x2C,0x2D,0x2E,0x2F,	// 0x20
	0x30,0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,0x3A,0x3B,0x3C,0x3D,0x3E,0x3F,	// 0x30
	...
	0x8A,0x8C,0x8D,0x8E,0x96,0x9A,0x9F,0x87,0x88,0x89,0x8A,0x8B,0x8C,0x8D,0x8E,0x8F,	// 0x80
	0x90,0x91,0x92,0x93,0x94,0x95,0x96,0x97,0x98,0x99,0x9A,0x9B,0x9C,0x9D,0x9E,0x9F,	// 0x90
	0xA0,0xA1,0xA2,0xA3,0xA4,0xA5,0xA6,0xA7,0xA8,0xA9,0xAA,0xAB,0xAC,0xAD,0xBE,0xBF,	// 0xA0
	0xB0,0xB1,0xB2,0xB3,0xB4,0xB5,0xB6,0xB7,0xB8,0xB9,0xBA,0xBB,0xBC,0xBD,0xBE,0xBF,	// 0xB0
	0xC0,0xC1,0xC2,0xC3,0xC4,0xC5,0xC6,0xC7,0xC8,0xC9,0xCA,0x88,0x8B,0x9B,0xCF,0xCF,	// 0xC0
	0xD0,0xD1,0xD2,0xD3,0xD4,0xD5,0xD6,0xD7,0xD8,0xD8,0xDA,0xDB,0xDC,0xDD,0xDE,0xDF,	// 0xD0
	0xE0,0xE1,0xE2,0xE3,0xE4,0x89,0x90,0x87,0x91,0x8F,0x92,0x94,0x95,0x93,0x97,0x99,	// 0xE0
	0xF0,0x98,0x9C,0x9E,0x9D,0xF5,0xF6,0xF7,0xF8,0xF9,0xFA,0xFB,0xFC,0xFD,0xFE,0xFF		// 0xF0
};

/*! ************************************

	\var const Word8 Burger::MacRomanUS::ToUTF8Table[128][4]
	\brief Table to convert MacRomanUS to UTF8.

	This 128x4 array holds the 128 high ascii codes for MacRomanUS converted
	to UTF8. All 128 codes are present in Unicode except for code 0xF0
	which is the closed Apple logo. The Apple logo is mapped to unicode 0xF8FF
	which appears to be the Apple default for Unicode.
...
const Word8 BURGER_ALIGN(Burger::MacRomanUS::ToUTF8Table[128][4],16) = {
...
/*! ************************************
	...
	MacRomanUS character. Codes 0 through 0x7f are considered ASCII while codes
	0x80 through 0xFF are mapped to MacRomanUS. If the UTF8 character cannot be mapped to
	MacRomanUS encoding, Burger::MacRomanUS::BAD will be returned instead.
	...
	\param pInput Pointer to UTF8 buffer that contains the 1 to 3 byte buffer to convert. \ref NULL will page fault.
	\return The unsigned 8 bit character code (0x00-0xFF) or Burger::MacRomanUS::BAD if the
		UTF8 value wasn't low ASCII and couldn't be mapped to MacRomanUS.
	\sa Burger::UTF8::NextToken(const char *) or Burger::UTF8::GetTokenSize(const char *)
	
***************************************/

Word BURGER_API Burger::MacRomanUS::FromUTF8(const char *pInput)
{
	...
	do {
		if ((First==pTable[0][0]) &&		// So far, so goo?
			(Second==pTable[0][1])) {
			Word Temp = pTable[0][2];	// Is this a 3 byte pattern?
			if (!Temp ||				// Nope, I'm good
				(reinterpret_cast<const Word8*>(pInput)[2] == Temp)) {	// Match?
				return 256-Counter;			// Return 0x80-0xFF
			}
		}
		++pTable;			// Next 4 byte entry
	} while (--Counter);
	return static_cast<Word>(Burger::MacRomanUS::BAD);		// Crud
}
...
/*! ************************************
	...
	into a MacRomanUS encoded "C" string. The function will return the size of the string
	...
	\param pOutput Pointer to byte buffer to receive the MacRomanUS encoded string. \ref NULL is okay if uOutputSize is zero, otherwise it will page fault.
...
Word BURGER_API Burger::MacRomanUS::FromUTF8(char *pOutput,WordPtr uOutputSize,const char *pInput)
{
			...
			if (First<0x80U) {		// ASCII?
				...
				do {
					if ((First==pTranslateTable2[0][0]) &&
						...
						Word Temp = pTranslateTable2[0][2];
						if (!Temp) {
							++pInput;		// 2 byte code
							break;
						}
						if ((reinterpret_cast<const Word8*>(pInput)[1] == Temp)) {
				...
				if (Counter) {					// Did I get a valid code?
...
/*! ************************************
	...
	to a MacRomanUS encoded "C" string. The function will return the size of the string
	after encoding. This size is valid, even if it exceeded the output buffer size.
	The output pointer and size can be \ref NULL to have this routine calculate the size
	of the possible output so the application can allocate a buffer large enough
	to hold it.
	...
	was parsed. Zeros will be placed in the MacRomanUS stream as is.
	
	\param pOutput Pointer to a byte buffer to receive the MacRomanUS string. \ref NULL is okay
...
Word BURGER_API Burger::MacRomanUS::FromUTF8(char *pOutput,WordPtr uOutputSize,const char *pInput,WordPtr uInputSize)
{
	Word8 *pWorkPtr = reinterpret_cast<Word8 *>(pOutput);	
			...
			if (First<0x80U) {		// ASCII?
				...
				do {
					if ((First==pTranslateTable2[0][0]) &&
						...
						if ((uInputSize>=3) &&
							(reinterpret_cast<const Word8*>(pInput)[1] == Temp)) {
