...
static const uint16_t g_CRC16IBMTable[256] = {
...
uint16_t BURGER_API Burger::CalcCRC16IBM(const void *pInput,uintptr_t uInputLength,uint16_t uCRC)
{
	// Ok to do the deed?
	if (pInput && uInputLength) {		
        do {
            uCRC = static_cast<uint16_t>(g_CRC16IBMTable[static_cast<uint8_t>(static_cast<const uint8_t *>(pInput)[0] ^ uCRC)] ^ (uCRC >> 8U));
...
void BURGER_API Burger::GenerateCRCTable(uint16_t *pOutput,uint16_t uPolynomial,uint_t bBitReverse)
{
	// Begin iteration
	uint_t uCount = 0;
	do {
		...
		uint32_t uValue;
		if (bBitReverse) {
			uValue = static_cast<uint32_t>(g_ReverseBits[uCount])<<8U;
		} else {
			uValue = uCount<<8U;
		}
		// Blend with 8 passes
