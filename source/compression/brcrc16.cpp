/***************************************
	...
	The table is generated by GenerateCRC16Table(Word16 *,Word16,Word)
...
static const Word16 g_CRC16IBMTable[256] = {
...
/*! ************************************
	...
	The polynomial format is X^16+X^15+X^2+X^0
	or 0x8005
...
Word16 BURGER_API Burger::CalcCRC16IBM(const void *pInput,WordPtr uInputLength,Word16 uCRC)
{
	// Ok to do the deed?
	if (pInput && uInputLength) {		
		do {
			uCRC = static_cast<Word16>(g_CRC16IBMTable[static_cast<Word8>(static_cast<const Word8 *>(pInput)[0]^uCRC)] ^ (uCRC>>8U));
...
/*! ************************************
	...
	The default polynomial format is X^16+X^15+X^2+X^0
	or 0x8005, however any value can be entered
	...
	\sa CalcCRC16(const void *,WordPtr,Word16)

***************************************/

void BURGER_API Burger::GenerateCRCTable(Word16 *pOutput,Word16 uPolynomial,Word bBitReverse)
{
	// Begin iteration
	Word uCount = 0;
	do {
		...
		Word32 uValue;
		if (bBitReverse) {
			uValue = static_cast<Word32>(g_ReverseBits[uCount])<<8U;
		} else {
			uValue = uCount<<8U;
		}
		// Blend with 8 passes
		Word uPasses = 8;
		do {
			uValue = (uValue<<1U) ^ ((uValue&(1U<<15U)) ? uPolynomial : 0);
		} while (--uPasses);

		// Store the final result (Bit reversed)
		if (bBitReverse) {
			uValue = BitReverse(uValue,16);
		}
		pOutput[uCount] = static_cast<Word16>(uValue);
	} while (++uCount<256U);
}
