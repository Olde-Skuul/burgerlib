...
// CRC32B table
//
static const uint32_t g_CRC32BTable[256] = {0x00000000, 0x77073096, 0xEE0E612C,
...
static const uint32_t g_CRC32Table[256] = {0x00000000, 0x04C11DB7, 0x09823B6E,
...
/*! ************************************
	...
	Calculates a 32-bit CRC, "CRC32B", which is the 32-bit Frame Check Sequence
	of ITU V.42. This is the CRC algorithm used for Zlib, PNG, MPEG-2, Gzip and
	others

	The polynomial format is
...
/*! ************************************
	...
	Calculates a 32-bit CRC, "CRC32", which is the 32-bit Frame Check Sequence
	of ITU V.42. This is the CRC algorithm used for Ethernet
...
/*! ************************************
	...
	The default polynomial format is
	X^32+X^26+X^23+X^22+X^16+X^12+X^11+X^10+X^8+X^7+X^5+X^4+X^2+X^1+X^0 or
	0x04C11DB7, however any value can be entered
	...
	uint32_t* pOutput, uint32_t uPolynomial, uint_t bBitReverse) BURGER_NOEXCEPT
{
	...
	do {
		...
		do {
			uValue =
				(uValue << 1U) ^ ((uValue & (1U << 31U)) ? uPolynomial : 0);
		} while (--uPasses);
