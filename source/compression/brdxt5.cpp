...
void Burger::Dxt5Packet_t::Decompress(RGBAWord8_t *pOutput,uintptr_t uStride) const
{
	// Array of alphas to decompress from (8 unique alphas)
	uint8_t Alphas[8];

	// First, let's create the alpha data
	uint_t uAlpha1 = m_uAlpha1;
	uint_t uAlpha2 = m_uAlpha2;
	
	// The input data are used as is
	Alphas[0] = static_cast<uint8_t>(uAlpha1);
	Alphas[1] = static_cast<uint8_t>(uAlpha2);
	...
	if (uAlpha1 <= uAlpha2) {
		...
		Alphas[2] = static_cast<uint8_t>(((4U*uAlpha1) + (uAlpha2)) / 5U);
		Alphas[3] = static_cast<uint8_t>(((3U*uAlpha1) + (2U*uAlpha2)) / 5U);
		Alphas[4] = static_cast<uint8_t>(((2U*uAlpha1) + (3U*uAlpha2)) / 5U);
		Alphas[5] = static_cast<uint8_t>(((uAlpha1) + (4U*uAlpha2)) / 5U);
		Alphas[6] = 0;
		Alphas[7] = 255;
	} else {
		// It's the 6 (8 really) entry alpha
		Alphas[2] = static_cast<uint8_t>(((6U*uAlpha1) + (uAlpha2)) / 7U);
		Alphas[3] = static_cast<uint8_t>(((5U*uAlpha1) + (2U*uAlpha2)) / 7U);
		Alphas[4] = static_cast<uint8_t>(((4U*uAlpha1) + (3U*uAlpha2)) / 7U);
		Alphas[5] = static_cast<uint8_t>(((3U*uAlpha1) + (4U*uAlpha2)) / 7U);
		Alphas[6] = static_cast<uint8_t>(((2U*uAlpha1) + (5U*uAlpha2)) / 7U);
		Alphas[7] = static_cast<uint8_t>(((uAlpha1) + (6U*uAlpha2)) / 7U);
	}
	...
	Colors[3].m_uRed = static_cast<uint8_t>(((uColor1) + (uColor2*2U))/3U);
	...
	const uint8_t (*pAlphaIndexes)[3] = m_uAlphaIndexes;
	...
	uint_t i = 16/8;			// 16 pixels, 8 pixels per loop
	do {
		// Create the little endian 24 bit value
		uint_t uAlphaIndex = static_cast<uint_t>(pAlphaIndexes[0][0] + (pAlphaIndexes[0][1]<<8U) + (pAlphaIndexes[0][2]<<16U));
		++pAlphaIndexes;
		uint_t j=8/4;			// 8 pixels, 4 pixels per loop
		do {
			// 4 2 bits per pixel indexes
			uint_t uIndex = pColorIndexes[0];
			++pColorIndexes;
			uint_t k=4;		// 4 pixels per loop
			do {
...
void BURGER_API Burger::DecompressImage(RGBAWord8_t *pOutput,uintptr_t uOutputStride,uint_t uWidth,uint_t uHeight,const Dxt5Packet_t *pInput,uintptr_t uInputStride)
{
	...
	if (uWidth && uHeight) {
		...
		do {
			...
			pInput = reinterpret_cast<const Dxt5Packet_t *>(reinterpret_cast<const uint8_t *>(pInput)+uInputStride);
