...
/*! ************************************
	...
	Invoke the SDBM (Service Data Base Management) hashing algorithm on the
	contents of a class using the default seed value.
...
/*! ************************************
	...
	\brief Case insensitive SDBM (Service Data Base Management) hash callback
		for HashMapShared

	Invoke the SDBM (Service Data Base Management) hashing algorithm on the
	contents of a class using the default seed value and convert all upper case
...
/*! ************************************
	...
	Invoke the DJB2 Additive hashing algorithm on the contents of a class
	using the default seed value and convert all upper case characters into
	lower case.
...
/*! ************************************
	...
	\brief Case insensitive DJB2 Exclusive Or hash callback for
	HashMapStringCase

	Invoke the case insensitive DJB2 Exclusive Or hashing algorithm on the
	contents of a \ref String class using the default seed value.
...
/*! ************************************
	...
	This is the base class for each data entry used by the HashMap /
	HashMapShared hash manager. These entries are used for managing the linked
	list for multiple hash items

	The derived class will append the key / value pairs to this class.

	An entry is considered initialized if the hash value is not \ref
	INVALID_HASH

	An entry is considered "dormant", that is, part of a linked list, yet must
	be skipped when the entry is not \ref EMPTY_RECORD and the hash is \ref
	INVALID_HASH.

	This is to support erasing entries during the use of an iterator without
	complicating the iterator code.
...
/*! ************************************
	...
	If the Entry is not used at all, it will return \ref TRUE. Otherwise, it
	returns \ref FALSE. It's possible the entry is invalid, however it can be
	still part of a linked list.
...
/*! ************************************
	...
	HashMap is a template class to quickly look up data chunks using a key
	value. To cut down on compile time and reduce code bloat, a majority of the
	runtime it contained in this class and the template creates a front end with
	only the minimum amount of code to support the class.

	From a users point of view, HashMap is a 100% template based class, from a
	code point of view, HashMap is a dispatcher to HashMapShared.
...
/*! ************************************
	...
	\fn Burger::HashMapShared::HashMapShared(uintptr_t uEntrySize,uintptr_t
		uFirstSize,uintptr_t uSecondOffset, TestProc
		pTestFunction, EntryConstructProc pEntryConstructFunction, EntryCopyProc
		pEntryCopyFunction,EntryInvalidateProc pEntryInvalidationFunction,
		HashProc pHashFunction)
	...
	\param pEntryConstructFunction Function that invokes the default
		constructors for the Entry
	\param pEntryCopyFunction Function that invokes the copy constructors for
		the Entry
	\param pEntryInvalidationFunction Function that invokes the destructors for
		the Entry
...
/*! ************************************
	...
	Hash the key and use the hash to look up the data in the entry table. If
	found, an entry index is returned. If not, \ref INVALID_INDEX is returned.
...
uintptr_t BURGER_API Burger::HashMapShared::FindIndex(
	const void* pKey) const BURGER_NOEXCEPT
{
	// Assume failure
	...
	if (pEntry) {
		...
		const uintptr_t uHash = ComputeHash(pKey);

		// Mask to the size of the array
		uintptr_t uIndex = uHash & m_uSizeMask;

		// Get the initial index
		pEntry = reinterpret_cast<const Entry*>(
			reinterpret_cast<const uint8_t*>(pEntry) + (uIndex * m_uEntrySize));
		if (!pEntry->IsEmpty()) {
			// Is this entry occupied by a ROOT linked list entry?
			if (pEntry->IsHashInvalid() ||
				((pEntry->m_uHashValue & m_uSizeMask) == uIndex)) {
				// Since this is a valid root entry, begin the scan!

				for (;;) {
					// Test the hash first, then the key to ensure there isn't a
					// hash collision

					// Note, pEntry+1 is used because the key is the data that
					// follows the base Entry class.
					if ((pEntry->m_uHashValue == uHash) &&
						m_pTestFunction(pEntry + 1, pKey)) {
						// Found it.
						uResult = uIndex;
						break;
					}
					// Keys are equal, but hash differs!
					// Can occur if the == operator allows equality for
					// incomplete data
					BURGER_ASSERT(pEntry->IsHashInvalid() ||
						!m_pTestFunction(pEntry + 1, pKey));
					...
					BURGER_ASSERT(
						(!pEntry->IsEmpty() || pEntry->IsHashInvalid()) &&
						((pEntry->m_uHashValue & m_uSizeMask) ==
							(uHash & m_uSizeMask)));
				}
			}
		}
...
/*! ************************************
	...
	This helper function assumes that there is no data already allocated by the
	HashMapShared class. It will allocate a buffer and then mark each entry as
	empty and initialize all of the internal variables.
	...
	uintptr_t uCount, uintptr_t uEntrySize) BURGER_NOEXCEPT
{
	...
	do {
		...
		pEntry = reinterpret_cast<Entry*>(
			reinterpret_cast<uint8_t*>(pEntry) + uEntrySize);
	} while (--uCount);
}
...
/*! ************************************
	...
	\note If the data buffer is already a comparable size, no action will be
		performed.

	\param uNewSize Number of entries to allocate (Will be rounded up to a power
		of 2)
...
void BURGER_API Burger::HashMapShared::CreateHashBuffer(
	...
	if (!uNewSize) {
		...
		if (!m_uSizeMask || (uNewSize != (m_uSizeMask + 1))) {
			...
			if (pOldEntries) {
				...
				do {
					// Every entry with a valid hash will be added to the new
					// hash
					if (!pTempEntry->IsHashInvalid()) {
						Add(pTempEntry + 1,
							reinterpret_cast<const uint8_t*>(pTempEntry) +
								m_uSecondOffset);
					...
					pTempEntry = reinterpret_cast<Entry*>(
						reinterpret_cast<uint8_t*>(pTempEntry) + uEntrySize);
				} while (--uCount);
...
/*! ************************************
	...
	Assuming a data entry is initialized, this function will remove it from the
	linked list and call the invalidation function.

	If the entry being erased is part of a linked list chain, it will be
	destroyed, but the linked list will be retained. This is to allow iterators
	to continue to function without error.
...
void BURGER_API Burger::HashMapShared::Erase(uintptr_t uIndex) BURGER_NOEXCEPT
{
	BURGER_ASSERT(m_pEntries && (uIndex <= m_uSizeMask));
	Entry* pEntryToErase = GetEntry(uIndex);

	// Get the root index entry
	const uintptr_t uTestIndex = pEntryToErase->m_uHashValue & m_uSizeMask;
...
void BURGER_API Burger::HashMapShared::Erase(const void* pKey) BURGER_NOEXCEPT
{
	// Find the entry
	const uintptr_t uIndex = FindIndex(pKey);
	// Valid?
...
/*! ************************************
	...
	Iterate over the hash data and find the index to the first entry that
	contains valid data. This is used by HashMap::begin() and HashMap::begin()
	const

	\return Index to the first valid entry or \ref INVALID_INDEX if no entries
		are valid.
...
uintptr_t BURGER_API Burger::HashMapShared::FindFirst(
...
/*! ************************************
	...
	Given a pointer to a key and the length in bytes of the key, call the hash
	algorithm using the stored function pointer. If it the very rare case that a
	hash matches \ref INVALID_HASH, it will be changed to \ref INVALID_HASH
	-0x8000 to "validate" the hash.
...
uintptr_t BURGER_API Burger::HashMapShared::ComputeHash(
	const void* pKey) const BURGER_NOEXCEPT
{
	// Create the hash
...
/*! ************************************
	...
	Clear out all the data in this hash and copy the entries from another hash
	into this one.
...
void BURGER_API Burger::HashMapShared::Copy(
	const HashMapShared* pInput) BURGER_NOEXCEPT
{
	Clear();
	...
	const uintptr_t uEntrySize = pInput->m_uEntrySize;
	m_uEntrySize = uEntrySize;
	...
	if (uCount) {
		...
		do {
			...
			if (!pEntry->IsHashInvalid()) {
				Add(pEntry + 1,
					reinterpret_cast<const uint8_t*>(pEntry) + m_uSecondOffset);
			}
			pEntry = reinterpret_cast<const Entry*>(
				reinterpret_cast<const uint8_t*>(pEntry) + uEntrySize);
		} while (--uCount);
	}
}
...
/*! ************************************
	...
	Expand the size of the hash if needed, and then insert a new key data pair
	into the hash. This function should now be called if a key should be
	replaced if present. Use the HashMap::Set() function for that.
...
void BURGER_API Burger::HashMapShared::Add(
	const void* pT, const void* pU) BURGER_NOEXCEPT
{
	...
	const uintptr_t uHash = ComputeHash(pT);
	const uintptr_t uIndex = uHash & m_uSizeMask;
	...
	if (pEntry->IsEmpty()) {
		...
		if ((pEntry->m_uHashValue & m_uSizeMask) == uIndex) {
			...
			m_pEntryCopyFunction(pNewEntry, pEntry + 1,
				reinterpret_cast<const uint8_t*>(pEntry) + m_uSecondOffset);
			...
			for (;;) {
				...
				if (pCollisionEntry->m_uNextInChain == uIndex) {
					...
					pNewEntry->m_uNextInChain = pEntry->m_uNextInChain;
					m_pEntryCopyFunction(pNewEntry, pEntry + 1,
						reinterpret_cast<const uint8_t*>(pEntry) +
							m_uSecondOffset);
...
/*! ************************************
	...
	and if found, return the pointer to the data pointed by the Entry pointer
	added with m_uSecondOffset.
...
const void* Burger::HashMapShared::GetData(const void* pT) const BURGER_NOEXCEPT
{
	const uintptr_t uIndex = FindIndex(pT);
	const void* pResult = NULL;
	if (uIndex != INVALID_INDEX) {
		pResult = reinterpret_cast<const uint8_t*>(GetEntry(uIndex)) +
			m_uSecondOffset;
	}
	return pResult;
}
...
/*! ************************************
	...
	\fn Burger::HashMapShared::Entry *Burger::HashMapShared::GetEntry(uintptr_t
		uIndex)
...
/*! ************************************
	...
	\fn const Burger::HashMapShared::Entry * \
		Burger::HashMapShared::GetEntry(uintptr_t uIndex) const
...
/*! ************************************
	...
	This class is the base class for iteration over a hash table using standard
	STL algorithms. It's not meant to be used by applications. A HashMap will
	derive from this class to inherit the common functionality and then provide
...
/*! ************************************
	...
	\fn Burger::HashMapShared::const_iterator::const_iterator(const
		HashMapShared *pParent,uintptr_t uIndex)
	...
	\param uIndex Index to start iteration from (Must an index to a valid entry
		or \ref INVALID_INDEX)
...
/*! ************************************
	...
	\fn const Burger::HashMapShared::Entry \
		*Burger::HashMapShared::const_iterator::GetPtr(void) const
...
/*! ************************************
	...
	\fn const Burger::HashMapShared::Entry \
		&Burger::HashMapShared::const_iterator::operator*() const
	...
	BURGER_NOEXCEPT
{
	...
	if (uIndex != INVALID_INDEX) {
		...
		const uintptr_t uEndMask = pParent->GetSizeMask();
		// Get the starting pointer
		const Entry* pEntry = pParent->GetEntry(uIndex);
		const uintptr_t uEntrySize = pParent->GetEntrySize();
		do {
			// End of the array?
			if (++uIndex > uEndMask) {
				// Force to invalid
...
/*! ************************************
	...
	\fn uint_t Burger::HashMapShared::const_iterator::operator==(const \
...
/*! ************************************
	...
	Iterate over all of the initialized entries and destroy any entry that has
	valid data using a function that will handle the derived class disposal.
...
void BURGER_API Burger::HashMapShared::Clear(void) BURGER_NOEXCEPT
{
	if (m_pEntryInvalidationFunction) {
		Entry* pEntry = static_cast<Entry*>(m_pEntries);
		if (pEntry) {
...
void BURGER_API Burger::HashMapShared::Resize(
	uintptr_t uNewSize) BURGER_NOEXCEPT
{
...
/*! ************************************
	...
	A non-destructive function to resize the hash to a size that has padding for
	new entries to be added with minimal memory allocations.
...
void BURGER_API Burger::HashMapShared::SetCapacity(
	uintptr_t uNewSize) BURGER_NOEXCEPT
{
...
/*! ************************************
	...
	When the hash buffer is created, it's set to a size that's a power of two
	and that value is stored as (size-1) to use as a wrap around mask. To get
	the hash size, take this value and add one to it.
...
/*! ************************************
	...
	HashMap is a template class to quickly look up data chunks using a key
	value. To cut down on compile time and reduce code bloat, a majority of the
	runtime it contained in a parent class HashMapShared and the template
	creates a front end with only the minimum amount of code to support the
	class.
...
/*! ************************************
	...
	Entry records have some extra data for connecting to the hash and also data
	for a copy of the key and data. This is declared in the template so it
	adjusts depending on the data needed
...
/*! ************************************
	...
	\note The const_iterator can only be created by a call to HashMap::begin()
		or HashMap::end()
...
/*! ************************************
	...
	\fn Burger::HashMap::const_iterator::const_iterator(const HashMapShared \
		*pParent,uintptr_t uIndex)
...
/*! ************************************
	...
	\fn const Burger::HashMap::Entry & \
		Burger::HashMap::const_iterator::operator*() const
...
/*! ************************************
	...
	\sa EntryConstructProc, Copy(HashMapShared::Entry *,const void *,const void
		*) or Invalidate(HashMapShared::Entry *)

***************************************/

/*! ************************************

	\fn void Burger::HashMap::Copy(HashMapShared::Entry *pEntry,const void \
		*pT,const void *pU)

	\brief Default copy constructor for an Entry

	Callback function to copy construct an uninitialized template Entry
		structure

	\param pEntry Pointer to an uninitialized Entry record
	\param pT Pointer to a key record to copy
	\param pU Pointer to a data record to copy
	\sa EntryCopyProc, Construct(HashMapShared::Entry *) or
		Invalidate(HashMapShared::Entry *)
...
/*! ************************************
	...
	\sa EntryInvalidateProc, Copy(HashMapShared::Entry *,const void *,const void
		*) or Construct(HashMapShared::Entry *)
...
/*! ************************************
	...
	\param pHashFunction Pointer to a hash function
	\param uDefault Number of entries to allocate (Will be rounded up to a power
		of 2) \sa HashMap(HashProc) or HashMap(const HashMap &)
...
/*! ************************************
	...
	\sa Clear(), HashMap(HashProc), HashMap(HashProc,uintptr_t) or HashMap(const
		HashMap &)
...
/*! ************************************
	...
	Delete all of the data and make a copy of another hash and place it in this
	class.
...
/*! ************************************
	...
	Using a key, look up the item in the hash and if present, return a reference
	to the data. If the entry didn't exist, create it with a default constructor
	for the data.
...
/*! ************************************
	...
	Using a key, look up the item in the hash and if present, replace the data
	with the a copy of the data passed. If the entry didn't exist, create it
	with a copy of the passed data.
...
/*! ************************************
	...
	\param rKey Reference to the key to look up
	\return \ref NULL if the key wasn't in the hash or a valid pointer to the
		data if so.
...
/*! ************************************
	...
	Return an STL compatible iterator pointing to the first entry in the hash
	table. The iterator can be set to end() if there is no data in the hash.
...
/*! ************************************
	...
	\param rKey Reference to the key to look up
	\return const_iterator preset to the terminator of the hash or the requested
		entry
...
/*! ************************************
	...
	HashMapString is a template class to quickly look up data chunks using a
	String as a key value.

	Unlike the standard HashMap which applies a hash to the class contents, this
	HashMap will use the string contained in the String class as the key data.

	From a users point of view, HashMapString is a 100% template based class,
	from a code point of view, HashMapString is a dispatcher to HashMapShared.
...
/*! ************************************
	...
	This internal function performs a case insensitive string comparison between
	two String classes.
...
/*! ************************************
	...
	HashMapStringCase is a template class to quickly look up data chunks using a
	String as a key value.

	Unlike the standard HashMap which applies a hash to the class contents, this
	HashMap will use the string contained in the String class as the key data.

	From a users point of view, HashMapStringCase is a 100% template based
	class, from a code point of view, HashMapStringCase is a dispatcher to
	HashMapShared.

	\note String hashing is case insensitive. For case sensitive hashing, use
		HashMapString

	\sa DJB2StringHashXorCaseFunctor, HashMapStringCaseTest(const void *,const
		void *), HashMapShared, HashMap or HashMapString
