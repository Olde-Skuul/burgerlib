...
uintptr_t BURGER_API Burger::File::Read(void* pOutput, uintptr_t uSize)
{
	...
	if (uSize && pOutput) {
		...
		if (fp) {
			// Read data
			size_t uRead = static_cast<size_t>(read(fp, pOutput, uSize));
			if (uRead == -1) {
				uRead = 0;
			}
	...
	const void* pInput, uintptr_t uSize) BURGER_NOEXCEPT
{
	...
	if (uSize && pInput) {
		...
		if (fp) {
			// Write data
			size_t uWrite = static_cast<size_t>(write(fp, pInput, uSize));
			if (uWrite == -1) {
				uWrite = 0;
			}
	...
	if (!pInput->StoreTimeT(&NewTime)) {
		...
		if (fp) {
			...
			if (iError != -1) {
				...
				// Access time
				Array[0].tv_sec = MyStat.st_atimespec.tv_sec;
				Array[0].tv_usec =
					static_cast<uint32_t>(MyStat.st_atimespec.tv_nsec / 1000);

				// Modification time
				Array[1].tv_sec = static_cast<time_t>(NewTime);
...
{
	uint_t uResult = kErrorNotSupportedOnThisPlatform;
#if 0
	HANDLE fp = m_pFile;
	if (fp) {
		...
		BOOL bFileInfoResult = SetFileTime(fp,&CreationTime, nullptr, nullptr);
		if (bFileInfoResult) {
			uResult = kErrorNone;
		}
	}
