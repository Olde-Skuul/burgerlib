...
/***************************************
	...
	Optimized and included into Burgerlib by Rebecca Ann Heineman <becky@burgerbecky.com>
...
static void *BURGER_API LibRefGetFunctionInLib(const char *pFilename,const char *pFunctionName)
{
	Burger::CodeLibrary MyLib;
	void *pFunction = NULL;

	if (MyLib.Init(pFilename)) {		/* Load the library */
		pFunction = MyLib.GetFunction(pFunctionName);		/* Load the function if found */
		if (!pFunction) {
