...
/*! ************************************
	...
	directories are stored with their Directory IDs and volume reference numbers.
	Since these number can ba invalidate when a directory is deleted or created,
	any call to a Burgerlib function that performs that action will also purge this
	cache.
	
	When a Burgerlib path is parsed, a Directory ID and Volume Reference needs to
	be generated. If the directory exists in the cache, the values found here will be
	used. If not, it will be determined and added to the cache.
...
/*! ************************************
	...
	\sa Burger::Filename::PurgeDirectoryCache(void) or Burger::Filename::ExpandCache_t
	\note Only available on MacOS
...
/*! ************************************
	...
	\sa Burger::Filename::InitDirectoryCache(void) or Burger::Filename::ExpandCache_t
	\note Only available on MacOS
...
const char * Burger::Filename::GetNative(void)
{
			...
			if (uDirLength>=2) {
				...
				do {
					...
					if (pCache->m_pName && 
						...
						// Partial match?
						if (!MemoryCaseCompare(pCache->m_pName,pPath,pCache->m_uNameLength)) {

							// If this is a full match, use it and exit immediately
							if (pCache->m_uNameLength==uDirLength) {
								// Grab the volume reference and directory ID
								MemoryCopy(m_FSRef,pCache->m_FSRef,sizeof(m_FSRef));

								// Mark this as the most recently used
								pCache->m_uHitTick = Tick::Read();

								// Length of the result (The leading colon is needed
								// since it tells Mac OS 9 this is a partial pathname)
								if (pFileName[0]==':') {
									++pFileName;
								}
								...
								if (uFinalLength<sizeof(m_NativeFilename)) {
									MemoryCopy(pNew,pFileName,uFinalLength+1);
								} else {
	...
	uDirLength = 0;
	if (pBiggestMatch && 0) {
		MemoryCopy(m_FSRef,pBiggestMatch->m_FSRef,sizeof(m_FSRef));		// Get the cache
		uDirLength = pBiggestMatch->m_uNameLength;
		pBiggestMatch->m_uHitTick = Tick::Read();
	} else {
		short sVRefNum = 0;		// Init the Macintosh volume numbers to the current working directory
		long lDirID = 0;
	...
	if (pFileName) {
		while (pWorkPath[0]) {
			...
			FSRef TempRef;
			{
				...
				eError = FSMakeFSRefUnicode(reinterpret_cast<FSRef *>(m_FSRef),Name16.GetLength(),Name16.GetPtr(),kUnicode16BitFormat,&TempRef);
			}
...
/* Now, just copy the rest of the path */
	...
	Length = static_cast<Word32>(Output-TempPath);		/* How many bytes is the new path? */
...
/*! ************************************
	...
	\sa Burger::Filename::SetDirID(long) or Burger::Filename::GetVRefNum(void) const
	\note Only available on MacOS and only valid after a call
		to \ref Burger::Filename::GetNative()
...
/*! ************************************
	...
	\sa Burger::Filename::SetVRefNum(short) or Burger::Filename::GetDirID(void) const
	\note Only available on MacOS and only valid after a call
		to \ref Burger::Filename::GetNative()
...
/*! ************************************
	...
	\sa Burger::Filename::GetDirID(void) const or Burger::Filename::SetVRefNum(short)
	\note Only available on MacOS
...
/*! ************************************
	...
	\sa Burger::Filename::GetVRefNum(void) const or Burger::Filename::SetDirID(long)
	\note Only available on MacOS
...
/*! ************************************
	...
	What is returned is either \ref NULL for an error condition, pOutput if the user supplied buffer
	was large enough to contain the requested data or a newly allocated pointer to a larger buffer
	to hold the requested data.
	
	\note If the returned pointer is not the pointer passed in via pOutput, dispose of it with
	a called to \ref Burger::Free(const void *) when done.
...
Word BURGER_API Burger::Filename::SetFromDirectoryID(long lDirID,short sVolRefNum) 
{
/*
...
*/
	...
	Clear();
	// FIrst step, create an FS Ref from the parent ID and volume reference number
	FSRef CurrentRef;
	FSSpec CurrentSpec;
	CurrentSpec.parID = lDirID;
