...
Word BURGER_API Burger::File::Open(Filename *pFileName,eFileAccess eAccess)
{
	...
	static const SInt8 g_Permissions[4] = { fsRdPerm,fsWrPerm,fsWrPerm,fsRdWrPerm };
	HFSUniStr255 ForkName;
	FSGetDataForkName(&ForkName);
	// Convert the filename to unicode
	String16 MyName(pFileName->GetNative());
	// Create a UFT16 FSRef
	OSErr eError = FSMakeFSRefUnicode(pFileName->GetFSRef(),MyName.GetLength(),MyName.GetPtr(),kUnicode16BitFormat,reinterpret_cast<FSRef *>(m_FSRef));
	Word uResult = FILENOTFOUND;

	// Error? File not found?
	// See if it should be created
	if ((eAccess!=READONLY) && (eError==fnfErr)) {
		...
		eError = FSCreateFileUnicode(pFileName->GetFSRef(),MyName.GetLength(),MyName.GetPtr(),
			kFSCatInfoTextEncoding + kFSCatInfoFinderInfo, &MyInfo,reinterpret_cast<FSRef *>(m_FSRef),NULL);
	}
	if (!eError) {
		short fp;
		eError = FSOpenFork(reinterpret_cast<FSRef *>(m_FSRef),ForkName.length,ForkName.unicode,g_Permissions[eAccess],&fp);
		if (!eError) {
...
WordPtr BURGER_API Burger::File::GetSize(void)
{
	...
	if (fp) {
		...
		if (!eSize) {
			if (static_cast<Word64>(lFileSize)<=static_cast<Word64>(0xFFFFFFFFU)) {
...
WordPtr BURGER_API Burger::File::Read(void *pOutput,WordPtr uSize)
{
	...
	if (uSize && pOutput) {
		...
		if (fp) {
			...
			OSErr eRead = FSReadFork(fp,fsAtMark,0,uSize,pOutput,&lDataRead);		// Read data
...
WordPtr BURGER_API Burger::File::Write(const void *pInput,WordPtr uSize)
{
	...
	if (uSize && pInput) {
		...
		if (fp) {
			...
			OSErr eWrite = FSWriteFork(fp,fsAtMark,0,uSize,pInput,&lDataWritten);		// Write data
...
WordPtr BURGER_API Burger::File::GetMark(void)
{
	...
	if (fp) {
		...
		if (!eErr) {
			if (static_cast<Word64>(lCurrentMark)<=static_cast<Word64>(0xFFFFFFFFU)) {
