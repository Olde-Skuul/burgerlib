...
Word Burger::TimeDate_t::Load(const UTCDateTime *pUTCDateTime)
{
	...
	if (!ConvertUTCToLocalDateTime(pUTCDateTime,&TempTime)) {
		...
		m_usMilliseconds = static_cast<Word16>((static_cast<Word>(TempTime.fraction) * 1000U)>>16U);
...
/*! ************************************
	...
	\param pUTCDateTime Pointer to a UTCDateTime structure to fill in with the time/date
...
/*! ************************************
	...
	Convert the timestamp value from the MacOS file system
	to a Burgerlib \ref TimeDate_t structure.
...
/*! ************************************
	...
	Convert the timestamp value to the MacOS file system
	from a Burgerlib \ref TimeDate_t structure.
